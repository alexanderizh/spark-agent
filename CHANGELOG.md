...
- **自定义工具配置改用代码编辑器**：扩展中心里工具源码、请求头 JSON、JSON Body 模板、提取规则 JSON、测试输入 JSON、输入参数 Schema 原文、cURL 与 OpenAPI 导入这 8 处普通输入框统一换成代码输入框。控件抽离为公用 `components/code-editor`（只复用代码视图已有的 Monaco 运行时与主题跟随，不动 `CodeViewerEditor`），带语法分色、行号与括号配色，字段高度仍按原来的 rows 口径换算，宽度随面板自适应。
- **JSON / TS 语法错误就地提示**：携带语言服务的字段在非法处显示波浪线，并在字段下方给出「第 N 行：…（共 N 处）」摘要；TS 只提示语法级错误（诊断码 1000–1999），语义类误报不进入摘要，避免在缺少工程上下文时把合法代码判成错误。JSON Body 模板另有专用「JSON 模板」语言：只着色、不带校验，`{{参数}}` 单独高亮，不再被内置 JSON 校验误判为非法。
- **快捷回复选项与关闭按钮并排自然换行**：去掉选项独立分组容器，关闭按钮作为最后一个元素参与同一换行流，一行放得下就跟在选项末尾，放不下跟着换行，不再被长选项挤到单独一行左侧。

## [0.13.4] - 2026-09-23

### 新增

- **账号同步余量**：设置页「立即同步」操作行新增余量行（单次上限 / 参照数据量 / 余量 + 3px 细进度条，余量低于 20% 转警告色），账号中心新增「账号同步」面板（单次上限、上次同步数据、上次同步时间、最近设备、上次结果）。上限口径是「单次请求体」而非累计配额，与服务端 `formatMib` 共用同一套体积格式化规则；服务端未返回上限时回退保守默认值并标注「服务端版本较低，仅供参考」。
- **同步前数据量测量**：新增「测量本次数据量」入口与 `account-sync:get-status` / `account-sync:estimate-payload` 两个 IPC 通道；测量走与同步一致的采集链路，只测量、不发送、不落库（提示词库封面需逐张压缩，故由用户显式触发而不是切换类别时自动跑）。

### 修复

- **同步超限改为本地拦下**：发送与预览前按服务端上限预检请求体字节数，超限直接提示「本次同步数据约 X，超过单次上限 Y」且不发请求；无法确认上限时跳过预检交由服务端裁决，避免用错误上限阻断合法同步。
- **网络抖动不再中断任务**：模型调用瞬时失败（连接失败、断流、429/5xx）的默认重试由 2 次提升到 10 次，且终止条件只按次数——渠道返回的 `Retry-After` 超长时截断到 `retry_max_delay_ms` 后继续重试，不再直接放弃该路由并以 `llm.retry_delay_exceeded` 结束整轮。
- **自动重试过程可见**：重试上报为轻量提示行（重试进度 / 本次等待 / 失败原因），多次尝试聚合为一行，不再静默等待；失败尝试已流出的正文会在重放前复位，不再与重放正文叠加。
- **已输出内容后断流可手动重试**：`llm.partial_stream_failed` 不再被标为不可重试，错误卡片恢复「重新尝试」入口，并展示引擎给出的恢复建议。
- **激活的历史会话发消息后浮到最新**（#196）：发送成功后即时刷新该会话的最后活动时间；同一轮内按 `turnId` 去重，thinking / 等待审批等连续非终态事件不再反复改写时间造成会话列表重排。
- **快捷回复不再跨会话残留**：切换会话时清空父级消息镜像，新会话不再沿用上一个会话解析出的快捷回复建议；选项独立成组换行，长选项以省略号收尾，关闭按钮不再被挤到单独一行。

### 改进

- **引擎诊断接入统一日志服务**：桌面端 spark 引擎执行器把统一日志注入引擎（含 LLM 重试层），重试与放弃决策、失败尝试输出摘要不再只落在 Electron 主进程看不到的 stderr。

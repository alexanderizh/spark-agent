...
- `apps/desktop/src/renderer/design/views/platform-model/ManagedModelPreferencesModal.tsx`：分区展示对话模型与自动启用的图片模型。
...
- [x] **Step 4: 运行 ProviderService 测试并确认通过**
...
- [x] **Step 4: 实现轻量同步、成功节流和 singleflight**
...
- [x] **Step 5: 运行 Hook 与现有 Provider UI 测试**
...
Expected: 无空白错误；变更仅包含本计划列出的当前项目文件以及用户原有、未被本任务改动的工作区文件。使用 GitNexus impact / detect-changes、直接调用点检索、测试和 `git diff` 共同核对影响范围。

...
- 使用**目标应用绑定**，Agent 只拥有用户明确选择的应用/窗口。
...
2. **目标校验会话化**：应用身份、窗口绑定、权限状态在会话开始时确认；目标未变化时不在每一步重复列举全部应用和窗口。
3. **动作校验轻量化**：低风险动作只验证目标仍绑定、元素/布局版本仍可用、用户未接管，不能同步执行完整审计流程。

...
> 实施进度（2026-08-01）：会话任务合同承载 L0/L1 会话授权；默认全桌面任务允许跨应用执行，显式 `targetWindowId` 任务才应用精确 app/window 隔离。普通导航/输入不创建审批；L2/L3 继续使用 digest-bound 单次 ticket，L4/unattended 继续 handoff。`codex-full-access` / `claude-bypass` 跳过逐动作审批与新应用首次动作升档，但仍保留系统权限、Host 信任、敏感凭据和不可逆动作边界。低风险证据保持异步，高风险动作在 ticket 消费和 backend 执行前同步 flush 当前 before-frame；落盘失败 action blocked、ticket 不消费。对抗测试与五轴安全审查已完成，阶段代码待独立提交。
...
- Full Access 对默认全桌面任务取消逐动作审批和应用白名单阻断；显式单窗口范围、系统权限、Host 信任、敏感凭据与不可逆动作边界仍保留。

...
`CanvasInlineNodeTitleEditor` 增加默认值为 `click` 的 `activation`，折叠卡传 `doubleClick`。`CanvasStage` 新增重命名回调并复用现有 `onUpdateNodeData` 保存颜色；Workspace 用 `patchNodes([nodeId], { title })` 保存标题。

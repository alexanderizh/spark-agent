...
Manifest 属性中的 `enum` 作为任务面板的推荐候选项展示；`examples` 只有在字段显式
声明 `x-allow-custom: true`、且已由 provider 契约确认安全时才进入候选项。这样不会把
通用 schema 中的示例误当成当前模型支持的尺寸或画幅。`x-allow-custom` 和 `pattern`
继续决定是否允许输入自定义值。OpenAI GPT Image 2 以及百炼 Wan/Qwen 图片模型的尺寸
字段因此可以同时提供已核对的常用尺寸、画幅对应的推荐项和 provider 原生格式的自定义
分辨率，候选项的显示标签与提交值可以分离。

火山 Seedream 的内置尺寸和自定义尺寸按具体版本的 manifest 分开声明，不能把某个版本的
`1K/2K/3K/4K` 或像素尺寸复制给其他版本。MiniMax `image-01` 同时展示官方枚举画幅和
满足约束的自定义宽高；`image-01-live` 只展示官方画幅，且不显示自定义宽高。百炼的
`宽*高`、火山和 OpenAI 的 `宽x高` 仅作为各自 schema 已声明的 provider 原生格式处理。

允许自定义的文本、尺寸和数值参数会按 operation、provider/model 身份、capability 和字段名
缓存最近使用值，作为下次输入的候选项；有限枚举值不会写入该缓存，切换到其他模型时不会
复用上一模型的自定义参数。
...
- “影视创作”统一按“文本编排 / 资产提取 / 视觉生成 / 视频生成”分组；“基础任务”统一按“图像 / 文本 / 视频 / 音频”分组。右键菜单、牵线菜单与节点浮动菜单使用相同分组和顺序，不再因入口不同而平铺或重新分类。
- 文本、Prompt、文本产物操作节点和可展开文本的组节点共享完整文本能力，不再按“普通文本 / 剧本 / 分镜脚本 / 功能节点”裁剪菜单。标准文本能力包括所有接受文本或文本附带图片、视频输入的基础任务。
- 普通文本可以直接创建“生成角色身份板”生图节点；存在角色资产时按角色批量创建，不存在角色资产时直接使用当前文本作为角色设定创建单个身份板任务。
- 操作节点按当前主产物的实际内容类型展示能力；只要主产物是文本，就与普通文本节点使用相同的影视创作和基础任务菜单。
- “影视创作”包含现有流水线动作以及“故事板”“360 全景图”。
- “基础任务”不再重复展示故事板和 360 全景图，只保留通用图像、文本、视频和音频任务。

...
- 分镜生成使用现有 `parseShotTable()` 可识别的 `{"shots": [...], "summary": {...}}` JSON。Markdown 表格由程序根据结构化结果生成，不再要求模型同时维护 JSON 和 Markdown 两份内容。
...
- 剧本：检查至少存在可识别场次标题和正文块；不满足时任务保留原始响应并标记格式错误，不创建 `pipelineRole: "screenplay"` 的产物节点。
- 分镜：使用 `parseShotTable()` 解析；没有有效镜头或必需字段缺失时不创建合法分镜节点，也不写入 `shotGroups`。
...
- 剧本或分镜解析失败时保留任务原始响应和错误摘要，不创建带专用 `pipelineRole` 的产物。

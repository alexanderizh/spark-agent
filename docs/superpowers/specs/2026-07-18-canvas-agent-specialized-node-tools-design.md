...
专用流水线动作的上游门槛只判断节点实际承载的媒体类型，不判断上游的
`pipelineRole`、影视资产 `metadata.kind`、分镜分组或场次来源。普通文本、章节、剧本和
其他文本节点都可以作为文本动作的输入；普通图片也可以作为声明支持图片的视觉动作输入。
`inputTypes` 是动作目录中唯一的输入类型门控，统一用于节点右键菜单、浮动工具栏和 Agent
能力发现。

`pipelineRole` 仍可用于产物语义、显示标签、专用 Prompt、输出物化和增强型批处理。例如已有
`shotGroupId`/`shotSegmentId` 时可以按分镜片段批量生成视频，但缺少这些回链不应阻断普通文本
或图片创建对应的待确认任务节点。真正必要的图片、视频、音频数量、角色和可读路径校验仍由
操作能力与任务提交校验保留。
...
- 剧本生成使用现有场次剧本格式：场号、内外景、地点、时间、出场人物、动作描述、角色对白和旁白；提示词明确给出 `第1场｜内景｜地点｜时间`、`场1 内景 地点 时间` 和 `INT. 地点 - 时间` 等可识别示例，未知字段允许留空。
...
- 剧本：兼容“第1场｜内景｜地点｜时间”、`场次/场景`、数字场号、Markdown 标题和 INT./EXT. 等写法，出场人物、动作、对白等缺失字段留空供编辑。非空响应无法识别场次时自动补入 `第1场｜｜｜` 后仍创建带 `pipelineRole: "screenplay"` 的可编辑剧本节点；只有空响应才标记 `invalid_screenplay_output`。原始模型文本仍单独保存在任务详情中。
...
- 普通文本可直接发现并创建分镜脚本、实体提取、分镜关键帧图、角色/场景/道具/特效图等
  专用任务；不因缺少剧本、分镜或影视资产语义标记而阻断。

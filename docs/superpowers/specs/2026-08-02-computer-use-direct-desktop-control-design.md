...
长任务不再把截图作为每轮模型决策的默认输入。观察中存在可操作的 AX/HTML 角色或稳定元素引用时，首先只发送应用、窗口、URL、界面树和元素引用；只有树决策无效、树信息不足、语义动作 noop，或验收必须依赖像素证据时，才把当前截图升级给同一决策模型。稳定的键盘输入、点击后输入、滚动和等待允许以 2–8 个动作的小批次执行，Host 仍在每个动作前校验目标绑定并在状态失效时停止批次。

Anthropic 兼容接口正常返回但在 JSON 外附带简短说明、Markdown fence，或省略 decision 外层而直接返回白名单动作时，由本地适配器提取并规范化；未知动作、畸形 payload 和越界坐标继续拒绝。此类失败诊断为本地 `decision_output_invalid`，不得误称模型不支持多模态。

截图证据持久化必须从 `computer_sessions` 解析真实 `session_id` 与 `turn_id`，禁止以空归属写入，否则数据库所有权触发器会拒绝每次观察。

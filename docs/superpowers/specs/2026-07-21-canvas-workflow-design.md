...
自动从节点内部创建的下游节点，统一按来源节点右侧的确定性布局落位：单个节点与来源节点中轴线对齐；多个节点先按网格或纵向间距规整排列，再以整个排列区域的中轴线与来源节点对齐。该类自动布局保留预设位置，不再被已有节点的碰撞避让逐个拆散；用户通过牵线释放创建的节点仍保留鼠标释放位置。
...
工作流从库中拖入画布后会物化为普通节点和普通连线，用户可以任意修改、删除、重连或新增节点；此后它不再依赖原工作流定义才能运行。画布上的临时节点流程通过多选任务节点的“批量提交运行”入口一键执行：选区内部的 `used_as_input` 连线会被拓扑分析为前台轻量 DAG，从根任务节点逐层提交，每层等待真实画布任务完成并刷新产物后再准备下游请求。
...
- 采用“定义运行”和“画布临时流程运行”双入口：保存的定义可由 `canvas_workflow_run` 冻结执行；已展开或手动搭建的普通节点图则通过选区批量提交按连线顺序运行。

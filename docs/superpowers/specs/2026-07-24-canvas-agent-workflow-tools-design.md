...
- Agent 创建完整多节点流程时优先使用 `canvas_create_reusable_workflow_graph`，由程序原子生成空媒体输入占位、操作节点、依赖连线和 DAG 布局；默认只生成真实画布节点，不自动保存工作流定义。
- `canvas_validate_workflow_graph` 只检查用户指定或当前选中的流程子图。输入节点可无入边、输出操作可无出边、`note` 可独立；中间操作必须位于输入到输出的有效路径中。

# SparkWork 工程运维

> 状态: 已落地 | 最后核对: 2026-09-22

这里集中放仓库作为 GitHub 开源项目运行时需要长期维护的约定：项目板、Issue/PR 流程、Wiki、GitHub Pages、CI 和发布链路。

## 文档索引

- [GitHub 治理与项目板](github-governance.md)
- [发布与 GitHub Pages 运维](release-and-pages.md)
- [运行架构](../architecture/overview.md)

## 仓库外资源

- [SparkWork · Open Source Roadmap](https://github.com/users/alexanderizh/projects/6)
- [GitHub Wiki](https://github.com/alexanderizh/spark-agent/wiki)
- [GitHub Pages 工程手册](https://alexanderizh.github.io/spark-agent/)
- [自有官网](https://www.yiqibyte.com/)

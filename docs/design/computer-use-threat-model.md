...
5. L4 动作只能拒绝或用户接管；L3 必须本地动作时确认；远程设备最多批准 L2。
6. 无本地问询通道、无人值守或远程断连时，审批必须暂停或拒绝，不能自动放行。
...
| T11  | Native Host 被替换或降级                     | 任意输入、虚假观察       | 签名、公证/hash、artifact manifest、协议兼容性检查                            | 安装/升级完整性测试    |
...
- L3/L4 策略存在漏拦截；
- Native Host 未签名、hash 未验证或协议允许未知字段；
- 原始快照、AX 文本、输入正文或 nonce 出现在普通日志；
- 无 verification record 仍可写入 `completed`；
- 无问询通道时 Computer/remote workflow 仍会自动批准；
- Kill Switch 注册失败但 My Desktop 仍可启用。

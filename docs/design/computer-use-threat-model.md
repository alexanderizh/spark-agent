...
| T21  | AppControlBridge 被扩展成通用 Renderer 后门  | 绕过 UI/Host 策略、任意执行 | 仅协议枚举 `set_theme/navigate/prefill_composer`；草稿只填空值且不发送，敏感值 L4 handoff；请求绑定 session/action/command；回执仅主窗口 mainFrame；禁止 eval/selector/URL/shell/任意 IPC；执行后重新观察 | schema/IPC/bridge 负向测试              |

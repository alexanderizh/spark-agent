...
- 会话运行时把非敏感的 Provider 路由和 Manifest 写入当前进程专属的临时 JSON 文件，子进程环境仅携带 `SPARK_MEDIA_CONFIG_FILE` 短路径与按 Provider 分离的短 API Key 变量。这避免完整 Manifest 使 Windows `CreateProcess` 环境块/命令行超限并导致 Claude/Codex SDK `spawn ENAMETOOLONG`。
- 运行时目录在 Unix 上使用 `0700`，配置文件使用 `0600`；Windows 继承用户临时目录 ACL。相同配置按 SHA-256 复用文件，应用正常退出时删除当前进程目录。API Key 不写入运行时文件、提示词或工具返回值。
...
- 使用大于 32KB 的 Manifest 配置验证 MCP 能通过运行时文件加载，且 SDK 子进程 env 不再包含 `SPARK_MEDIA_*_JSON`。

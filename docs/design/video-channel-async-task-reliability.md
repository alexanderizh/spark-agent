...
- 轮询类任务在提交接口返回渠道任务 ID 后立即产生 `provider-submitted` 生命周期日志，并将脱敏后的提交响应与 HTTP 请求/响应摘要推送到画布；无需等待轮询结束即可在任务详情查看渠道任务 ID。
- 任务详情将提交响应与最终轮询响应分开展示；后续 running 回执不会再被内部 runtime task ID 覆盖。
...
- 后台任务提交响应即时回写、渠道任务 ID 优先级和任务详情持久化测试。

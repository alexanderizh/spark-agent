...
让用户通过 Agent 对话配置应用未内置的图片、音频和视频渠道。Agent 负责收集渠道资料、读取真实官方文档、生成完整 Contract V2、保存 Provider，并分阶段调试；不绕过现有 ProviderService、Keychain、Manifest 校验器和画布运行时。
...
3. 每个模型生成一个完整 Manifest。相同 `modelId` 可以存在于不同 Provider，但 Manifest ID 必须使用 `custom:<model-slug>:<随机实例后缀>`。
...
- 新建配置强制渠道唯一 Manifest ID；更新历史 `custom:<modelId>` 配置时仅警告，保证旧配置可继续编辑和运行。
...
日志只记录动作、Provider ID、模型数、能力数、阶段、状态码和脱敏错误摘要。测试覆盖唯一 ID、重复模型、请求预览、协议保留、Keychain 输入不回显、模型发现、付费调用确认、真实调用脱敏、MCP 契约及 PlatformBridge 路由。

...
- 已新增 `tencent-tokenhub` Provider、专用异步 Adapter、21 个图片/视频 Manifest 和内置预设。
- 视频请求按 HY/YT-FX/HumanActor/Kling/Vidu 模型族组装；统一使用 `POST /v1/api/video/submit|query`。
- Kling 9 个 TokenHub 模型均已录入文生/图生能力及代际参数差异；v3 时长完整覆盖 3–15 秒。
- YT-Video-FX 已录入官方页面当前 141 个唯一 template 值；HumanActor 已支持图片 + 音频输入。
- Adapter 已接入共享参数编译器，执行 defaults、schema、aliases、strict 裁剪，并由腾讯专属 validator 处理分镜、首尾帧和参数互斥规则。
- Kling Omni、动作控制、视频延长属于 `vclm.tencentcloudapi.com` 旧签名 API；TokenHub 模型清单未声明对应能力，因此本次只保留文档，不在 TokenHub 渠道伪启用。
...
- 目前未使用真实 TokenHub API Key 做计费冒烟；Kling/Vidu 参数映射依据 TokenHub 模型清单和 1616 原接口参数的小写下划线规则，首次上线仍需逐模型小流量验证。

...
平台模型 ID 可用性与 tags 的合并结果是映射关系的唯一来源。登录 bootstrap、应用恢复已有登录态以及 Provider 页面事件刷新都会重新合并目录：普通条目写入文本 `modelIds`，图片条目写入 `mediaModelRefs`。任一目录请求失败都保留当前 Provider，不会把暂时缺少 tags 的图片模型误写回聊天列表。旧版本中误存进聊天 `modelIds` 的图片模型会自动迁移；原有 Provider、用户自定义渠道及所有已有适配器配置不需要迁移。

...
| `seed-tts-2.0-standard`（异步长文本默认版本） | TTS 异步长文本合成 | 提交：`POST https://openspeech.bytedance.com/api/v3/tts/submit`；查询：`POST https://openspeech.bytedance.com/api/v3/tts/query` | `X-Api-App-Id` + `X-Api-Access-Key`（旧版控制台双头，区别于单向流式/音频生成的 `X-Api-Key` 单头） | 提交：`speaker`、`text`、`audio_params`；查询：`task_id` | 异步（submit + poll query） | `mp3`/`pcm`/`ogg_opus`/`wav` | https://www.volcengine.com/docs/6561/1829010 |

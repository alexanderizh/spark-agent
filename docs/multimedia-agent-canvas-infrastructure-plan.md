...
- 无限画布操作节点已升级为“稳定步骤节点 + 内嵌产物展示”：视图层按同一操作节点的 `taskId` 与 `generated` 血缘聚合多次运行，默认展示最新运行，可切换历史运行，并在一次运行包含多个产物时用页点切换。底层 `CanvasTask`、`CanvasAsset`、独立产物节点和血缘边仍保持分离，因而任务队列、审计、拖拽、连线和独立编辑均兼容现状。操作节点和独立产物节点统一为扁平深色卡片；新操作节点默认尺寸为 `560×360`，已有图片/视频操作节点会按媒体宽高比计算“媒体正文 + 24px 头部 + 运行切换区”的展示高度，独立媒体节点继续使用包含头部高度的比例拟合尺寸。

...
- 无限画布操作节点已升级为“稳定步骤节点 + 内嵌产物展示”：视图层按同一操作节点的 `taskId` 与 `generated` 血缘聚合多次运行，默认展示最新运行，可切换历史运行，并在一次运行包含多个产物时用页点切换。底层 `CanvasTask`、`CanvasAsset`、产物节点和血缘边仍保持分离，以兼容任务队列、审计、资产管理和导出；任务直属的 `generated` 产物节点默认只作为虚拟内嵌产物，不再在画布上重复渲染，已有下游连线会在显示层折叠回任务节点。用户仍可从资源库显式拖入同一资产，创建可独立编辑和连线的实体节点。操作节点和实体产物节点统一为扁平深色卡片；新操作节点默认尺寸为 `560×360`，已有图片/视频操作节点会按媒体宽高比计算“媒体正文 + 24px 头部 + 运行切换区”的展示高度，独立媒体节点继续使用包含头部高度的比例拟合尺寸。
- 操作节点内嵌产物已补齐类型化展示和双击工作台：文本产物会区分分镜脚本、普通 JSON 与普通正文，分镜脚本在节点卡片和详情中恢复为结构化表格；图片、视频、音频使用匹配的媒体预览。双击已有产物的任务节点默认进入“产物”页，可在多次运行和单次多产物之间切换，并切换到“任务配置”页微调参数；文本/分镜产物可继续编辑，媒体支持下载，360 产物保留全景预览，只有资产记录而没有实体节点的角色/场景/道具等产物可跳转资源库继续管理。复杂操作集中在工作台，右键菜单只保留“打开工作台 / 展开主产物 / 展开最近运行 / 展开全部历史”的轻量产物入口，避免重复堆叠整套资源菜单。
- 操作节点与资源节点的合一已补齐产物语义模型：步骤节点可按 `single`、`candidates`、`collection`、`bundle` 组织运行产物，并用 `primaryOutputId` 区分“默认资源操作项”与生产状态。普通多图/多视频结果默认是候选型，只把主产物传给下游；角色提取、场景提取、剧本拆解、分镜展开等工作流是集合型，会把最近一次运行的完整集合传给下游，不强迫用户在集合成员中只采用一个。
- 产物工作台已增加“运行历史”页、主产物/默认预览设置、显式多选与批量展开。用户可展开当前产物、本次运行或全部历史；展开动作基于同一 `assetId` 创建资产引用节点，不复制底层资产，并写入 `materializedOutput` 回链。重复展开会聚焦已有引用节点，操作节点到引用节点使用 `references`，因此不会再次被“任务内嵌产物”投影隐藏。
- 完成后的操作节点会按当前主产物获得图片、视频、文本、全景、资源库和剧本流水线能力：节点管理操作仍作用于稳定步骤节点，下载、标注、宫格切分、生产状态等资源操作作用于主产物；后续 AI 任务仍连接稳定步骤节点，再由输入解析器按候选/集合语义解析为真实产物，避免流程连线绑定到容易变化的临时结果节点。
- 无限画布手动连线已持久化：素材/Prompt 连到操作节点会同步为 `used_as_input` 并写入任务输入；其它手工连线统一保存为 `references`。`generated` 只允许任务完成回写时显式创建，不再因为连线源头是操作节点而误判为“系统生成血缘”。
...
- 无限画布上传图片已支持多选：一次选择多张图片会按网格创建图片节点，并自动包装为 group 节点；图片仍作为真实 parent/child 子节点显示在组内，可拖拽、可参与 AI 输入展开，组背景不额外复制缩略图。
- 视频生成/编辑的首帧、尾帧选择已扩展到全画布图片节点：AI 操作面板不再只使用当前选中图片作为候选，面板内上传会复用“添加图片节点”链路，上传后新图片进入画布并被选中。面板会读取当前模型 capability 的 `input.maxImages`，当前内置视频 I2V 多为 1 张图、视频编辑多为 2 张图；若后续模型声明 `maxImages > 2`，面板会开放额外参考图多选。对只支持单图的视频模型，多图参考应先走画布 `image_compose` / 图生图合成生成一张新的图片节点，再作为首帧输入视频任务。
- 无限画布右侧栏已改为项目内 tab 面板：`属性` 保留任务队列、项目文件夹和节点 Inspector；`项目信息` 提供项目基础统计、项目文件夹入口，以及项目统一提示词 / 反向提示词设置。AI 操作弹窗可由用户勾选是否注入项目统一提示词，并可把反向提示词传入 `negativePrompt`，用于约束生成内容中不能出现或不能执行的事项。
- 无限画布右侧栏的 `项目信息` 现已补充“应用级节点预设”：可按 operation 配置跨项目生效的预置提示词、预置反向提示词和默认参数；`panorama_360` 内置默认会固定 `aspect_ratio=2:1`（并保留 `resolution=2k`），保证 360 全景任务节点默认符合等距柱状投影要求。
- 无限画布 AI 操作弹窗的提示词输入已封装为 `CanvasPromptEditor`：弹窗宽度扩展到 780px，主输入更名为“提示词”，支持 Markdown 富文本工具、预览、展开编辑、独立反向提示词输入，以及右下角 `AI 优化` 按钮。优化按钮复用现有 `prompt_optimize` 画布任务能力，把当前提示词和反向提示词交给 agent/provider 生成可继续编辑的提示词节点。
...
- 公共流程模板提取、类型化输入/输出端口、流程参数暴露，以及跨运行分支的可视化对比等生产工作台能力。
...
- 项目资产抽屉提供单项下载入口，下载不修改项目快照；本地项目资源走文件复制，文本/Prompt 资产保存为文本文件。
...
2. 无限画布中选中 prompt/image 后，只展示可用操作；提交后在稳定步骤节点内显示任务进度、运行历史和产物，必要时可把任意产物展开为资产引用节点。

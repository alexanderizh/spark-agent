...
- 普通回答、思考、工具块之间保留纵向留白；底部状态栏只显示模型、权限值、推理强度值、性能、目录和 `/help`，行首不再画固定竖线，只留 2 格对齐用留白（与输入框的 `❯ ` 提示符对齐），turn 或自更新运行中在该位置显示 loading 指示。
- 运行中的忙碌行（`请求模型` / `正在思考` / `运行 <工具>` 等当前动作）同样以 loading 指示开头，运行细节跟在后面；没有排队、重试或中断细节时不再残留分隔符。

## 代码与 diff 着色（2026-09-20）

TUI 的代码与补丁展示统一走 `src/tui/syntax.ts`（分词）与 `src/tui/diff.ts`（补丁归类），渲染入口在 `src/tui/components/code-segments.tsx`：

- 回答中的 fenced code 按语言着色，已支持 `ts`/`js`/`tsx`/`jsx`、`py`、`sh`/`bash`/`zsh`、`json`、`rs`、`go`、`yaml`、`toml`、`sql`；未识别语言保持原有 dim 样式，不猜色。
- 归一化 diff 用增删色区分：显式 `diff`/`patch` fence、无语言但含 `@@`/`+++` 文件头的 fence、`edit` 工具成功后的有界补丁预览（带头部 `+n -m` 计数）、以及工具结果预览里的 diff 片段。文件头/`index` 行为 dim，`@@` 为 hunk 色，`+`/`-` 分别为增/删色，上下文行保持默认前景色。
- `edit` 补丁预览最多 14 行：窗口保留改动区域前后各 3 行上下文，超出预算时截断并追加“其余 N 行未展开”，但 `+n -m` 计数始终反映整次编辑。
- 着色严格保持字符：分词只改颜色，不改文本、宽度与缩进，`tokenizeLine(line)` 的结果拼接后必须逐字等于输入行（有测试锁定），因此着色不会影响折行、滚动视口或 golden frame 对齐。
- 调色板走 `TuiTheme.code`（可选）：未提供该字段的旧主题按 `defaultCodePalette` 回退，16 色/单色终端由终端自身降级，文本不变。

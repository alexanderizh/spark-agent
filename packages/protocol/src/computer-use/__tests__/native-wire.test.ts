...
describe('native host wire protocol', () => {
  ...
  it('treats the action-result execution channel as optional and validates its values', () => {
    ...
    // Legacy host reply without channel reporting stays valid.
    expect(schema.parse(base)).toEqual(base)
    expect(schema.parse({ ...base, executionChannel: 'background_ax' })).toEqual({
      ...base,
      executionChannel: 'background_ax',
    })
    expect(schema.parse({ ...base, executionChannel: 'foreground_cg' })).toEqual({
      ...base,
      executionChannel: 'foreground_cg',
    })
    expect(schema.parse({ ...base, executionChannel: null })).toEqual({
      ...base,
      executionChannel: null,
    })
    expect(schema.safeParse({ ...base, executionChannel: 'hid_injection' }).success).toBe(false)
  })

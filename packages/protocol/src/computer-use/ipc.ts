...
export interface ComputerUseNativeHostDiagnosticReport {
  ...
  /**
   * Execution-transport outcome counters (background AX vs foreground HID). Optional so
   * reports persisted before channel reporting existed stay valid.
   */
  executionChannels?: ComputerUseExecutionChannelReport
}

export interface ComputerUseExecutionChannelReport {
  total: number

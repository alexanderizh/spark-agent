...
export const GITHUB_CONNECTOR_MANIFEST: ConnectorProviderManifest = {
  ...
  auth: [
    {
      ...
      label: 'GitHub OAuth + PKCE',
      description: '桌面端推荐登录流；用系统浏览器完成授权，token 由主进程 keystore 保存。',
      recommendedFor: ['desktop'],
      secretStorage: 'keystore',
      scopes: ['read:user', 'repo', 'workflow'],
      authorizationUrl: 'https://github.com/login/oauth/authorize',
      tokenUrl: 'https://github.com/login/oauth/access_token',
      callbackPath: '/connectors/github/oauth/callback',
      docsUrl:
    ...
    },
    {
      ...
      description: '适合 CLI、远程终端或无浏览器回调环境。',
      recommendedFor: ['headless', 'development'],
      secretStorage: 'keystore',
    ...
    {
      method: 'github-app',
      flow: 'app-installation-token',
      label: 'GitHub App Installation',
      description: '团队/组织生产集成推荐；使用短期 installation token 与细粒度仓库授权。',
      recommendedFor: ['team'],
      secretStorage: 'vault',
      scopes: ['metadata:read', 'contents:read', 'issues:read', 'pull_requests:read'],
      installationUrl: 'https://github.com/apps',
      docsUrl:
        'https://docs.github.com/en/apps/creating-github-apps/authenticating-with-a-github-app/generating-an-installation-access-token',
    },
    {
      method: 'pat',
      flow: 'personal-access-token',
      label: 'Fine-grained Personal Access Token',
      description: '开发期和私有部署可用；生产连接建议迁移到 OAuth 或 GitHub App。',
  ...
  security: {
    preferredAuthFlow: 'authorization-code-pkce',
    tokenStorage: 'keystore',
    supportsPkce: true,
    supportsDeviceFlow: true,
    supportsInstallationTokens: true,
    notes: [
      'PAT 仅用于开发期或手动接入，界面不持久化明文 token。',
      '团队/组织生产集成优先使用 GitHub App installation token。',

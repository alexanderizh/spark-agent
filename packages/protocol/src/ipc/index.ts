...
export interface SessionCreateRequest {
  ...
  /** OpenAI Fast mode (request-level service_tier=fast), independent from reasoning effort. */
  fastMode?: boolean
...
export interface SessionReorderQueuedTurnsRequest {
  sessionId: SessionId
  turnIds: string[]
}

export interface SessionReorderQueuedTurnsResponse {
  changed: boolean
  running: boolean
  queuedTurns: SessionQueuedTurn[]
}
...
export interface CommandExecuteRequest {
  sessionId: string
  message: string
  /** 命令入队时冻结的会话运行时；forwardToAgent 时继续使用同一份快照。 */
  providerProfileId?: string
  modelId?: string | null
  agentId?: string
  agentAdapter?: SessionAgentAdapter
...
/** `browser-panel:window-open` — 把内置浏览器面板切换为独立窗口（携带当前页 URL） */
...
/** `browser-panel:window-restore-panel` — 独立浏览器窗口收回为右侧面板 */
...
/** `browser-panel:pick-to-composer` — 独立窗口内拾取的元素引用转发回主窗口会话输入框 */
export interface BrowserPanelPickToComposerRequest {
  /** BrowserElementReference 的 JSON 序列化（渲染端解析还原） */
  referenceJson: string
}

export interface BrowserPanelPickToComposerResponse {
  success: boolean
}
...
export interface WindowEnsureWidthRequest {
  /**
   * 期望的窗口最小宽度，CSS 像素（renderer 布局空间的临界值）。
   * 主进程会按当前 zoomFactor 换算成屏幕像素再与 bounds 比较。
   */
  minWidth: number
  allowShrink?: boolean
  /**
...
export interface IpcChannelMap
  ...
  'session:reorder-queued-turns': [
    SessionReorderQueuedTurnsRequest,
  ...
  'image:process': [ImageProcessRequest, ImageProcessResponse]
  ...
  'browser-panel:window-open': [BrowserPanelWindowOpenRequest, BrowserPanelWindowOpenResponse]
  'browser-panel:window-restore-panel': [
    BrowserPanelRestorePanelRequest,
    BrowserPanelRestorePanelResponse,
  ]
  'browser-panel:pick-to-composer': [
    BrowserPanelPickToComposerRequest,
    BrowserPanelPickToComposerResponse,
  ]
...
export interface IpcStreamChannelMap {
  ...
  /** 图片处理进度推送（按 requestId 关联请求）*/
  'stream:image:process-progress': ImageProcessProgress
  /** 通用二进制产物下载进度推送 */
  ...
  'stream:browser-panel:restore': {
    url?: string
  }
  /** 独立浏览器窗口内拾取的元素引用（主进程转发 → 主窗口渲染端，
   *  渲染端 JSON.parse 还原为引用 tag 加入输入框）*/
  'stream:browser-panel:element-picked': {
    referenceJson: string
  }
  /** 独立浏览器窗口已存在时的导航请求（主进程 → popout 渲染端）*/
  'stream:browser-window:navigate': {
    url: string
  }
  /** 内置浏览器 webview 内页面请求新窗口（window.open / target=_blank）：
   *  主进程统一 deny 并按宿主窗口推送，渲染端转成新 tab。*/
...
export interface AuthUploadFileRequest {
  ...
  mimeType?: string
  /**
   * 上传用途标识（如 'media-transfer' 表示渠道中转的临时文件）。
   * 'media-transfer' 时客户端以 ?tmp=1 调用 /upload：文件落服务端临时桶，
   * 由 MinIO Lifecycle 定期自动清理；其他用途（含未传）落主桶永久保留。
   */
  purpose?: string
}

...
export interface EvidenceCostSnapshot {
  ...
  costs: CostEvent[]
  /** Total cost events in the discussion; costs contains the first page. */
  costsTotal?: number
  /** Whether another cost page is available after the returned costs. */
...
export const EvidenceCostIpcSchemaRegistry = {
  'evidence-cost:get': z
    .object({ sessionId: z.string().uuid(), expectedDiscussionId: ID })
    .strict(),
  'evidence-cost:evidence:add': z
    .object({
      ...
      links: boundedList(
        z
          .object({ type: z.enum(['claim', 'task', 'handoff', 'deliberation', 'ledger']), id: ID })
          .strict(),
      ),
      source: z
        .object({
    ...
    .strict(),
  'evidence-cost:evidence:verify': z
    .object({
      sessionId: z.string().uuid(),
      expectedDiscussionId: ID,
      opId: ID,
    ...
    .strict(),
  'evidence-cost:evidence:invalidate': z
    .object({
      sessionId: z.string().uuid(),
      expectedDiscussionId: ID,
      opId: ID,
...
export function aggregateCost(events: CostEvent[]): CostAggregate[] {
  const dimensions: Array<[CostDimension, (event: CostEvent) => string | null]> = [
    ['room', (event) => event.roomId],
    ['task', (event) => event.taskId],
    ['agent', (event) => event.agentId],
    ['dispatch', (event) => event.dispatchId],
  ]
  return dimensions.flatMap(([dimension, keyOf]) => {
    const groups = new Map<string, CostEvent[]>()
    for (const event of events) {
      const key = keyOf(event)
      if (key != null) groups.set(key, [...(groups.get(key) ?? []), event])
    }
    return [...groups].map(([key, group]) => ({
      dimension,
      key,
      tokens: group.some((e) => e.tokens == null)
        ? null
        : group.reduce((sum, e) => sum + (e.tokens ?? 0), 0),
      amount: group.some((e) => e.amount == null)
        ? null
        : group.reduce((sum, e) => sum + (e.amount ?? 0), 0),
      latencyMs: group.some((e) => e.latencyMs == null)
        ? null
        : group.reduce((sum, e) => sum + (e.latencyMs ?? 0), 0),
      eventCount: group.length,
      unknown: group.some((e) => e.status === 'unknown' || e.tokens == null || e.amount == null),
    }))
  })
}

...
import { BrowserPanelDevtoolsIpcSchemaRegistry } from '../browser-panel-devtools.js'
...
export const IpcSchemaRegistry = {
  ...
  ...BrowserPanelDevtoolsIpcSchemaRegistry,
  ...
  'window:ensure-width': z.object({
    // CSS 像素（renderer 布局空间）；主进程按 zoomFactor 换算成屏幕像素再比较。
  ...
  // image:probe 与 image:process 共享 ImageProcessRequest 结构（sharp 实现，无 ffmpeg 依赖）。
  // params 宽松校验，数值范围在主进程 imageProcessHandler 内做。
  'image:probe': z.object({
    operation: z.literal('probe'),
    input: z.string().min(1).max(4096),
    params: z.record(z.string(), z.unknown()),
    requestId: z.string().min(1).max(100),
  }),
  'image:process': z.object({
    operation: z.enum(['probe', 'scaleCompress']),
    input: z.string().min(1).max(4096),
    params: z.record(z.string(), z.unknown()),
    requestId: z.string().min(1).max(100),
  }),
  ...
  'browser-panel:window-open': z.object({
    url: z.string().max(20_000).optional(),
  }),
  'browser-panel:window-restore-panel': z.object({
    url: z.string().max(20_000).optional(),
  }),
  'browser-panel:pick-to-composer': z.object({
    referenceJson: z.string().min(2).max(20_000),
  }),

...
describe('EvidenceCostService', () => {
  ...
  function evidence(service: EvidenceCostService, id = 'evidence-1', opId = `add-${id}`) {
    return service.addEvidence({
      ...
      source: { type: 'test', ref: 'test:evidence' },
      version: 'v1',
      summary: 'Summary',
      hash: 'sha256:x',
      opId,
  ...
  it('scopes evidence and cost reads, supports idempotency, and rejects op collisions', () => {
    ...
    const cost = service.recordUsage({
      id: 'usage-1',
      taskId: 'task-1',
      agentId: 'agent-a',
    ...
    expect(() =>
      service.recordUsage({ id: 'usage-2', tokens: 99, status: 'recorded', opId: 'usage-1-op' }),
    ).toThrow(EvidenceCostConflictError)
  ...
  it('enforces agent permissions, evidence CAS transitions, and invalidation audit', () => {
    ...
    expect(() =>
      agent.verifyEvidence({ id: created.id, expectedVersion: 1, opId: 'verify-agent' }),
    ).toThrow(/agent/i)
    ...
    expect(() =>
      user.invalidateEvidence({
        id: created.id,
        expectedVersion: 1,
        reason: 'stale',
        opId: 'invalidate-stale',
      }),
    ).toThrow(/version/i)
    const invalid = user.invalidateEvidence({
      id: created.id,
      expectedVersion: 2,
      reason: 'stale',
      opId: 'invalidate-1',
  ...
  it('enforces evidence quota and bounded list results', () => {
    ...
    for (let index = 0; index < 100; index += 1)
      evidence(service, `evidence-${index}`, `add-${index}`)
  ...
  it('aggregates each usage once and preserves unknown values', () => {
    ...
    const task = service
      .aggregate()
      .find((item) => item.dimension === 'task' && item.key === 'task-1')
    ...
    expect(
      service
        .aggregate()
        .filter((item) => item.dimension === 'room' && item.key === scope.roomId)[0],
    ).toMatchObject({ eventCount: 2, tokens: null })
  })

  it('paginates cost reads while aggregating the complete discussion history', () => {
    const service = EvidenceCostService.forSystem(db, scope)
    ...
    const page = service.listCostsPage(100, 0)
    expect(page).toMatchObject({ total: 101, hasMore: true })
    expect(page.items).toHaveLength(100)
    expect(service.listCostsPage(100, 100)).toMatchObject({ total: 101, hasMore: false })
    expect(
      service.aggregate().find((item) => item.dimension === 'task' && item.key === 'task-all'),
    ).toMatchObject({ eventCount: 101, tokens: 101 })
  })
  ...
  it('uses budget CAS/idempotency, disallows agents, and cleans every session table', () => {
    ...
    expect(
      db.raw
        .prepare('SELECT COUNT(*) AS count FROM evidence_cost_evidence_events WHERE session_id=?')
        .get(scope.sessionId),
    ).toEqual({ count: 0 })
  })
})

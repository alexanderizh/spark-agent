...
export class CanvasProjectRepository extends BaseRepository {
  ...
  list(userId = 0, includeDeleted = false): CanvasProjectRow[] {
    const sql = includeDeleted
      ? `SELECT * FROM canvas_projects WHERE user_id = ? ORDER BY datetime(last_opened_at) DESC`
      : `SELECT * FROM canvas_projects WHERE user_id = ? AND status != 'deleted' ORDER BY datetime(last_opened_at) DESC`

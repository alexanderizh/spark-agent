...
describe('ModelCapabilityRegistry', () => {
  it('resolves provider-prefixed and family model ids', () => {
    expect(
      ModelCapabilityRegistry.getCapabilities('anthropic/claude-sonnet-4.5')?.contextWindow,
    ).toBe(200_000)
    expect(ModelCapabilityRegistry.getCapabilities('openai/gpt-5-codex')?.contextWindow).toBe(
      400_000,
    )
    expect(
      ModelCapabilityRegistry.getCapabilities('google/gemini-2.5-pro-preview')?.contextWindow,
    ).toBe(1_048_576)
  })
  ...
  it('keeps the managed platform default at 1M', () => {
    expect(resolveModelContextWindowForProvider('new-platform-model', true, undefined, {})).toBe(
      PLATFORM_MANAGED_DEFAULT_CONTEXT_WINDOW,
    )
  })

  it('prefers a model-level context window and falls back for unconfigured models', () => {
    const modelContextWindows = {
      'glm-5': 1_000_000,
      'deepseek-v4': 256_000,
    }

    expect(resolveModelContextWindowForProvider('glm-5', false, 400_000, modelContextWindows)).toBe(
      1_000_000,
    )
    expect(
      resolveModelContextWindowForProvider('qwen3.6-plus', false, 400_000, modelContextWindows),
    ).toBe(400_000)
    expect(
      resolveModelContextWindowForProvider('qwen3.6-plus', true, undefined, modelContextWindows),
    ).toBe(1_000_000)
  })
})

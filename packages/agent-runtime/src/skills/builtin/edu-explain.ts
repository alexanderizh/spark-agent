/**
 * Built-in Skill: 专题讲解生成
 *
 * 通过 5 步流程（理解→研究→验证→脚本→输出）生成高质量专题讲解内容，
 * 支持 HTML 幻灯片、自定义网页、PPTX、DOCX 等多格式输出。
 */
import type { SkillDefinition } from '../types.js'

export const eduExplainSkill: SkillDefinition = {
  id: 'builtin:edu-explain',
  name: '专题讲解生成',
  description:
    '根据题目/知识点，通过完整的 5 步流程（理解→研究→验证→脚本→输出）生成高质量专题讲解内容，支持 HTML 幻灯片、自定义网页、PPTX、DOCX 多格式输出',
  version: '1.0.0',
  author: 'Spark AI',
  category: 'writing',
  icon: 'GraduationCap',
  tags: ['education', 'explain', 'courseware', 'html', 'pptx', 'docx', 'slides'],

  requiredTools: ['Read', 'Write', 'Bash', 'WebSearch', 'WebFetch'],
  ...
  parameters: [
    {
      ...
      type: 'select',
      label: '输出格式',
      description: '选择生成的内容产物格式，可多选（逗号分隔）',
      defaultValue: 'html',
      options: [
        { label: 'HTML 幻灯片', value: 'html' },
        { label: '自定义网页', value: 'custom_html' },
        { label: 'PPTX 演示文稿', value: 'ppt' },
    ...
    },
    {
      ...
      type: 'select',
      label: '讲解风格',
      description: '选择讲解的风格模式',
      defaultValue: 'standard',
      options: [
        { label: '标准讲解', value: 'standard' },
        { label: '启发引导', value: 'heuristic' },
        { label: '深度拓展', value: 'competition' },
      ],
    },
    {
      name: 'includeQuiz',
      type: 'boolean',
      label: '包含测验题',
      description: '是否在讲解末尾追加测验题章节',
      defaultValue: false,
    },
  ],

  systemPrompt: `# 专题讲解生成系统

...
describe('spark_media MCP server', () => {
    ...
    server = createServer((req, res) => {
      if (
        ...
        req.on('end', () => {
          ...
          const currentPostedBody = JSON.parse(Buffer.concat(chunks).toString('utf8')) as Record<
            string,
            unknown
          >
      ...
      if (req.method === 'POST' && req.url === '/custom-edits') {
        // Contract V2 自定义 multipart 渠道（如 ToApis gpt-image-2）的图生图端点：
        // body 是 multipart 而非 JSON，捕获原始字节用于断言。
        const chunks: Buffer[] = []
        req.on('data', (chunk) => chunks.push(Buffer.from(chunk)))
        req.on('end', () => {
          postedPath = req.url ?? ''
      ...
      if (req.method === 'GET' && req.url === '/asset2.png') {
        res.writeHead(200, { 'content-type': 'image/png' })
        res.end(Buffer.from('second-reference-image'))
        return
      }
  ...
  it('compiles custom Contract V2 multipart image edits with remote reference URLs', async () => {
    // ToApis 等自定义渠道的 image.edit 按 OpenAI /v1/images/edits 规范声明为
    // multipart 文件上传；两张远程参考图必须被下载为文件 part 而非报错。
    const manifest = {
      id: 'custom:gpt-image-2',
      providerKind: 'custom',
      modelId: 'gpt-image-2',
    ...
    child = spawn(process.execPath, [path.resolve('src/tools/media-generation-mcp-server.mjs')], {
      ...
      env: {
        ...process.env,
        SPARK_MEDIA_API_KEY: 'custom-test-key',
        SPARK_MEDIA_PROVIDER: 'custom',
        SPARK_MEDIA_MODEL: 'gpt-image-2',
        SPARK_MEDIA_BASE_URL: baseUrl,
        SPARK_MEDIA_OUTPUT_DIR: tmpDir,
        SPARK_MEDIA_MANIFESTS_JSON: JSON.stringify([manifest]),
      },
    ...
    const response = await callMcp(child, {
      ...
      params: {
        ...
        arguments: {
          ...
          imageUrls: [`${baseUrl}/asset.png`, `${baseUrl}/asset2.png`],
        },
      },
    })

    expect(response.error).toBeUndefined()
    expect(postedPath).toBe('/custom-edits')
    expect(String(postedHeaders['content-type'])).toContain('multipart/form-data; boundary=')
    expect(postedRawBody).toContain('name="prompt"')
    expect(postedRawBody).toContain('merge the two references')
    expect(postedRawBody).toContain('name="image[]"')
    expect(postedRawBody).toContain('filename="asset.png"')
    expect(postedRawBody).toContain('filename="asset2.png"')
    expect(postedRawBody).toContain('second-reference-image')
    expect(existsSync(response.result.structuredContent.files[0])).toBe(true)
  })

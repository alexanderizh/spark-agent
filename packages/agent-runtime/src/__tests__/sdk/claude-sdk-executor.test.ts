...
describe('ClaudeSDKExecutor', () => {
  ...
  describe('buildPromptWithAttachments image routing', () => {
    ...
    it('keeps direct Read guidance for a couple of images', () => {
      const prompt = buildPromptWithAttachments('hello', [image('a.png'), image('b.png')])
      expect(prompt).toContain('use Read on the path')
      expect(prompt).not.toContain('Task subagents')
    })

    it('routes 3+ images to Task subagents instead of direct Reads', () => {
      const prompt = buildPromptWithAttachments('hello', [
        image('a.png'),
        image('b.png'),
        image('c.png'),
      ])
      // 多图指引必须同时给出：隔离原因 + 子代理路径 + compact 后禁重读
      expect(prompt).toContain('Analyze the images through Task subagents')
      expect(prompt).toContain('Do NOT Read the images directly')
      expect(prompt).toContain('do NOT re-Read images')
      expect(prompt).not.toContain('use Read on the path')
      // 附件路径本身仍要列出（compact 后凭路径可追溯）
      expect(prompt).toContain('/tmp/c.png')
    })

    it('passes the message through unchanged when no attachments exist', () => {
  ...
  describe('isImageFileInput', () => {
    it('matches bitmap extensions case-insensitively and rejects text/svg/missing paths', () => {
      expect(isImageFileInput({ file_path: '/tmp/shot.PNG' })).toBe(true)
      expect(isImageFileInput({ file_path: '/tmp/shot.jpeg' })).toBe(true)
      expect(isImageFileInput({ file_path: '/tmp/icon.svg' })).toBe(false)
      expect(isImageFileInput({ file_path: '/tmp/notes.txt' })).toBe(false)
      expect(isImageFileInput({ file_path: '/tmp/no-extension' })).toBe(false)

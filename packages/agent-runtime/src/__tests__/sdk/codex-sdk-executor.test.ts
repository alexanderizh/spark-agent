...
describe('CodexSdkExecutor', () => {
  ...
  it('injects Fast mode into Codex SDK config independently from reasoning effort', async () => {
    runStreamed.mockResolvedValue({ events: streamFrom([]) })
    ...
    await new CodexSdkExecutor().executeTurn(
      ...
      'turn-1',
      'hello',
      makeConfig({ fastMode: true, reasoningEffort: 'high' }),
    )

    expect(codexCtor).toHaveBeenCalledWith(
      expect.objectContaining({
        config: expect.objectContaining({ service_tier: 'fast' }),
      }),
    )
    expect(startThread).toHaveBeenCalledWith(

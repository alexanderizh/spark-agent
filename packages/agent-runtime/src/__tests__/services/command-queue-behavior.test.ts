...
describe('命令在运行中会话的排队行为', () => {
  ...
  it('运行中发送 /status：命令入队不注入事件；turn 结束后自动出队执行', async () => {
    ...
    await waitUntil(
      () =>
        persisted().some(
          (r) =>
            r.type === 'user_message' && r.content === '/status' && r.turnId !== turnARow?.turnId,
        ),
      5000,
  ...
  it('运行中发送命令时把完整运行时选择冻结在命令队列项内', async () => {
    const executorA = await startHeldTurn('turn A working')
    const runtimePatch = {
      providerProfileId: 'provider-cmd-queue',
      modelId: 'claude-sonnet-5',
      agentId: 'platform-manager-agent',
      agentAdapter: 'claude-sdk' as const,
      permissionMode: 'claude-auto' as const,
    ...
    const pendingTurns = (
      service as unknown as {
        pendingTurns: Map<string, Array<{ runtimePatch?: Record<string, unknown> }>>
      }
    ).pendingTurns.get(sessionId)
    expect(pendingTurns?.[0]?.runtimePatch).toEqual(runtimePatch)

    executorA.release()
    await waitUntil(
      () => service.getQueueState({ sessionId }).queuedTurns.length === 0,
      5000,
      'runtime-snapshotted command drained',
    )
  })

  it('立即执行命令时先应用运行时快照', async () => {
    const runtimePatch = {
      providerProfileId: 'provider-cmd-queue',
      modelId: 'claude-sonnet-5',
      permissionMode: 'claude-auto' as const,
    ...
    const session = new SessionRepository(db).findByIdOrFail(sessionId)
    expect(session.permission_mode).toBe('claude-auto')
    expect(session.reasoning_effort).toBe('high')
    expect(JSON.parse(session.metadata_json)).toMatchObject({ cliSparkOverride: null })
  })

  it('命令出队转 Agent 时先应用并继续传递自己的 runtimePatch', async () => {
    const runtimePatch = {
      providerProfileId: 'provider-cmd-queue',
      modelId: 'claude-sonnet-5',
      agentId: 'platform-manager-agent',
    ...
    const privateService = service as unknown as {
      commandController: {
        executeCommandAsEvents: (params: {
          sessionId: string
          message: string
        }) => Promise<{ isCommand: boolean; forwardToAgent?: boolean }>
      }
      executeQueuedCommandTurn: (
        sessionId: string,
        turn: {
          turnId: string
          message: string
    ...
    vi.spyOn(privateService.commandController, 'executeCommandAsEvents').mockResolvedValue({
      isCommand: true,
      forwardToAgent: true,
    })
    const sendTurn = vi.spyOn(service, 'sendTurn').mockResolvedValue({
      turnId: 'forwarded-turn',
      started: false,
    })

    await privateService.executeQueuedCommandTurn(sessionId, {
      turnId: 'queued-command',
      message: '/compact',
  ...
  it('运行中发送带 followUpPrompt 的命令：命令入队，turn 结束后命令执行且 follow-up 起跑', async () => {
    ...
    const commandRows = persisted().filter((r) => r.turnId !== turnARow?.turnId)
    expect(commandRows.some((r) => r.type === 'user_message' && r.content === '/validate')).toBe(
      true,
    )

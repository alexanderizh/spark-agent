...
describe('ProviderService', () => {
  ...
  it('preserves media ref adapterMode and capabilityOverrides through create and list', async () => {
    const profile = await service.createProvider({
      name: 'Gateway Aliases',
      provider: 'anthropic',
      defaultModel: 'gw-image',
      modelIds: ['gw-image'],
      apiEndpoint: 'https://gateway.example',
      apiKey: 'sk-gateway',
      modelType: 'image',
      mediaProvider: 'apimart',
      mediaApiType: 'auto',
      ...
      mediaModelRefs: [
        {
          ...
          modelId: 'gw-image',
          templateManifestId: 'apimart:gpt-image-2',
          displayName: 'Gateway Image',
          enabled: true,
          adapterMode: 'template',
          capabilityOverrides: {
            'image.generate': { defaults: { quality: 'high' } },
          },
        },
      ],
    })

    const stored = profile.mediaModelRefs?.[0]
    expect(stored?.adapterMode).toBe('template')
    expect(stored?.capabilityOverrides).toEqual({
      'image.generate': { defaults: { quality: 'high' } },
    })

    const listed = await service.listProviders()
    const ref = listed.find((item) => item.id === profile.id)?.mediaModelRefs?.[0]
    expect(ref?.adapterMode).toBe('template')
    expect(ref?.capabilityOverrides).toEqual({
      'image.generate': { defaults: { quality: 'high' } },
    })
  })

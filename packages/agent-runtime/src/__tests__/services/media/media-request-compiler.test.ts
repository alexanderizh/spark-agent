...
describe('compileMediaRequest — Documented Scenarios', () => {
  ...
  it('scenario 5b: explicit passthrough.enabled=false denies unknown fields even in compat mode', () => {
    const result = compile({
      capability: imageCapability({
        paramPolicy: { strict: false, passthrough: { enabled: false } },
      }),
      modelParams: { style: 'cinematic' },
    })
    expect(result.providerParams).not.toHaveProperty('style')
    expect(result.droppedParams.some((d) => d.name === 'style' && d.reason === 'unsupported_by_model')).toBe(true)
  })

  it('scenario 5c: compat mode without passthrough declaration still passes unknown scalars', () => {
    const result = compile({
      capability: imageCapability({
        paramPolicy: { strict: false },
      }),
      modelParams: { style: 'cinematic' },
    })
    expect(result.providerParams.style).toBe('cinematic')
  ...
  it('scenario 5d: explicit passthrough.enabled=true passes unknown scalars even under strict', () => {
    const result = compile({
      capability: imageCapability({
        paramPolicy: { strict: true, passthrough: { enabled: true } },
      }),
      modelParams: { style: 'cinematic' },
    })
    expect(result.providerParams.style).toBe('cinematic')

...
describe('media invocation compiler', () => {
  ...
  it('encodes query auth and never emits the API key as a custom header', async () => {
    ...
    expect(result.url).toBe(
      'https://provider.example/tasks/%7BtaskId%7D?verbose=true&token=secret-token',
    )
  ...
  it('runs the declared upload stage and exposes extracted URLs', async () => {
    ...
    const fetchImpl: typeof fetch = async (input, init) => {
      ...
      return new Response(
        JSON.stringify({ data: { url: 'https://files.example/reference.png' } }),
        {
          status: 200,
  ...
  it('supports batch multipart uploads and best-effort cleanup', async () => {
    ...
    const fetchImpl: typeof fetch = async (input, init) => {
      ...
      if (url.endsWith('/uploads/images')) {
        return new Response(
          JSON.stringify({
            data: [{ url: 'https://files.example/1.png' }, { url: 'https://files.example/2.png' }],
          }),
          { status: 200 },
        )
      }
    ...
    expect(result.images?.urls).toEqual([
    ...
    expect(calls[1]).toMatchObject({
      url: 'https://provider.example/uploads/cleanup',
      method: 'DELETE',
    })
  })

  it('downloads remote http(s) input URLs for multipart file parts', async () => {
    const fetchImpl: typeof fetch = async (input) => {
      const url = String(input)
      if (url === 'https://cdn.example/ref-a.png') {
        return new Response(Buffer.from('ref-a-bytes'), {
          status: 200,
          headers: { 'content-type': 'image/png' },
        })
      }
      if (url === 'https://cdn.example/ref-b.jpg') {
        return new Response(Buffer.from('ref-b-bytes'), {
          status: 200,
    ...
    const result = await compileInvocationRequest(
      {
        ...
        body: {
          ...
          parts: [
            ...
            { name: 'image[]', kind: 'file', value: '{{images}}' },
          ],
        },
      },
      {
        ...
        variables: {
          ...
          images: ['https://cdn.example/ref-a.png', 'https://cdn.example/ref-b.jpg'],
        },
        inputFiles: [
          {
            type: 'image',
            role: 'reference',
            url: 'https://cdn.example/ref-a.png',
            mimeType: 'image/png',
          },
          {
            type: 'image',
            role: 'reference',
            url: 'https://cdn.example/ref-b.jpg',
            mimeType: 'image/jpeg',
          },
        ],
        fetchImpl,
    ...
    const body = Buffer.from(result.body as Uint8Array).toString('utf8')
    expect(body).toContain('ref-a-bytes')
    expect(body).toContain('ref-b-bytes')
    expect(body).toContain('filename="ref-a.png"')
    expect(body).toContain('filename="ref-b.jpg"')
  })

  it('reports artifact_download_failed when a remote input URL cannot be fetched', async () => {
      ...
      compileInvocationRequest(
        {
          ...
          body: {
            kind: 'multipart',
            parts: [{ name: 'image', kind: 'file', value: '{{images}}' }],
          },
        },
        {
          apiEndpoint: 'https://provider.example',
          apiKey: 'secret-token',
          variables: { images: ['https://cdn.example/gone.png'] },
          inputFiles: [
            {
              type: 'image',
              role: 'reference',

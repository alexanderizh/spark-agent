...
const EVIDENCE = z
  ...
  .strict()
const ALTERNATIVE = z
  .object({ title: TEXT, summary: TEXT, tradeoffs: z.array(TEXT).max(8) })
  .strict()
const RISK = z
  .object({
    title: TEXT,
  ...
  .strict()
const LEDGER_VALUE = z.unknown().superRefine((value, context) => {
...
export class TeamDeliberationRuntimeAdapter {
  ...
  constructor(
    db: SparkDatabase,
    private readonly context: TeamDeliberationRuntimeContext,
  ) {
    const scope = {
    ...
    const options =
      context.ledgerWriter == null ? undefined : { ledgerWriter: context.ledgerWriter }
    this.service =
  ...
  buildToolDefinitions(): TeamToolDefinition[] {
    ...
    const propose: TeamToolDefinition = {
      ...
      handler: async (args) =>
        this.mutate(args, createInput, (input) =>
          this.service.create({
            ...input,
            proposal: input.proposal as DeliberationProposal,
    ...
    const addEvidence: TeamToolDefinition = {
      ...
      handler: async (args) =>
        this.mutate(args, evidenceInput, (input) =>
    ...
    const addAlternative: TeamToolDefinition = {
      ...
      handler: async (args) =>
        this.mutate(args, alternativeInput, (input) =>
          this.service.addAlternative({
            id: input.id,
            expectedVersion: input.expectedVersion,
    ...
    const addRisk: TeamToolDefinition = {
      ...
      handler: async (args) =>
        this.mutate(args, riskInput, (input) =>
          this.service.addRisk({
            id: input.id,
            expectedVersion: input.expectedVersion,
            risk: input.risk,
            opId: operationId(this.context.actorId, input.opId),
    ...
    const vote: TeamToolDefinition = {
      ...
      handler: async (args) =>
        this.mutate(args, voteInput, (input) =>
          this.service.addEvidence({
            id: input.id,
            expectedVersion: input.expectedVersion,
            evidence: {
    ...
    const decide: TeamToolDefinition = {
      ...
      handler: async (args) =>
        this.mutate(args, decideInput, (input) =>
          this.service.decide({
            id: input.id,
            expectedVersion: input.expectedVersion,
            decision: {
              ...
              ledgerWrite:
                input.decision.ledgerWrite == null
                  ? null
                  : {
    ...
    const resolve: TeamToolDefinition = {
      ...
      handler: async (args) =>
        this.mutate(args, resolveInput, (input) =>
  ...
  private async read(args: Record<string, unknown>): Promise<TeamToolHandlerResult> {
    if (Object.keys(args).length !== 0)
      return this.error('Deliberation read does not accept scope or other arguments.')
  ...
  private async mutate<T extends z.ZodTypeAny>(
    ...
    schema: T,
    build: (input: z.infer<T>) => DeliberationRecord,
  ...
  const visit = (current: unknown, depth: number): void => {
    ...
    if (typeof current === 'number') {
      if (!Number.isFinite(current))
        throw new Error('Deliberation JSON must contain finite numbers')

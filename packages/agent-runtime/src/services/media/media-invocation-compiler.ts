  ...
  for (const spec of specs ?? []) {
    ...
    if (spec.constraints?.maxBytes != null) {
      for (const file of files) {
        const buffer = await valueToBuffer(file.path ?? file.dataUrl ?? file.url ?? '', [file])
...
async function compileBody(
  ...
  if (body.kind === 'binary') {
    ...
    const buffer = await valueToBuffer(value, context.inputFiles)
...
function fileNameForValue(value: unknown, inputFiles: MediaInputFile[] | undefined): string {
  const file = findInputFile(value, inputFiles)
  return file?.path ? path.basename(file.path) : 'upload.bin'
}

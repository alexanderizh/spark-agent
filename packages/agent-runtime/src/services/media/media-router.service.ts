...
export class MediaRouterService {
  ...
  private prefersReferenceCapability(input: MediaGenerateInput): boolean {
    ...
    if (input.operation === 'text_to_video') {
      return files.some((file) => isMediaInputFile(file))
    }
    if (files.some((file) => isMediaInputFile(file) && file.type !== 'image')) return true
    const images = files.filter((file) => isMediaInputFile(file) && file.type === 'image')
...
/**
 * 与渲染端 canvasMediaContract.isMediaFile 同口径：type 为 file 但 mimeType 是
 * 媒体类型的输入也算媒体文件，保证两端 reference-capability 启发式一致。
 */
function isMediaInputFile(file: MediaInputFile): boolean {

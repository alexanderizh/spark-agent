  ...
  for (const ref of refs) {
    ...
    const effectiveManifest = manifest
      ? applyCapabilityOverrides(
          overlayRefDefaults(manifest, ref.defaults),
...
/**
 * 把 ref.defaults（用户/Agent 配置的按模型默认参数）叠加进各 capability 的 defaults。
 * 只叠加该 capability paramSchema 已声明的键，避免平面默认值跨能力污染（如把
 * image 的 size 写进 image.edit）；叠加后画布播种与运行时 mergeDefaults 读同一份
 * capability.defaults，保证「看到的默认值 = 实际生效的默认值」。
 * 顺序：先叠 ref.defaults，再由 applyCapabilityOverrides 覆盖更具体的按能力覆写。
 */
function overlayRefDefaults(
  manifest: MediaModelManifest,
  defaults: Record<string, unknown> | undefined,
): MediaModelManifest {
  ...
  const capabilities = manifest.capabilities.map((capability) => {
    ...
    return {
      ...capability,
      defaults: { ...(capability.defaults ?? {}), ...Object.fromEntries(applicable) },
    }
  })
  return changed ? { ...manifest, capabilities } : manifest
}

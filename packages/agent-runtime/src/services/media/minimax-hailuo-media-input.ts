  ...
  if (allowMmFile) {
    ...
    try {
      ...
      if (cached?.kind === 'file_id') {
        log.debug(`event=resolved kind=${kind} transport=mm_file source=transfer-cache`)
        return `mm_file://${cached.fileId}`
      }

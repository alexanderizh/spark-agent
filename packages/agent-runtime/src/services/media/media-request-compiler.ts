...
function filterCanonicalParams(
  ...
  // passthrough.enabled 显式声明时双向优先于 strict：
  //   - strict:false（兼容模式）下显式 enabled:false 拒绝未知字段；
  //   - strict:true 下显式 enabled:true 放行未知字段（与 allow 列表同款穿透语义）。
  // strict 仅在 passthrough.enabled 未声明时作为默认值来源。
  const passthroughEnabled = policy.passthrough?.enabled ?? !strict

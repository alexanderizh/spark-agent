...
export class TeamEvidenceCostRuntimeAdapter {
  ...
  constructor(
    db: SparkDatabase,
    private readonly context: TeamEvidenceCostRuntimeContext,
  ) {
    const scope = {
  ...
  buildToolDefinitions(): TeamToolDefinition[] {
    const read: TeamToolDefinition = {
      ...
      description:
        'Read evidence, usage events, aggregates, and budget for the current team discussion.',
      schema: {},
    ...
    const addEvidence: TeamToolDefinition = {
      ...
      handler: async (args) =>
        this.mutate(args, evidenceAddInput, (input) =>
    ...
    const recordUsage: TeamToolDefinition = {
      ...
      description:
        'Record actual usage. Unknown values remain unknown and are never estimated here.',
      schema: usageInput.shape,
      handler: async (args) =>
        this.mutate(args, usageInput, (input) =>
    ...
    const verify: TeamToolDefinition = {
      ...
      handler: async (args) =>
        this.mutate(args, evidenceVersionedInput, (input) =>
          this.service.verifyEvidence({
            ...input,
            opId: operationId(this.context.actorId, input.opId),
          }),
        ),
    ...
    const invalidate: TeamToolDefinition = {
      ...
      handler: async (args) =>
        this.mutate(args, evidenceInvalidateInput, (input) =>
          this.service.invalidateEvidence({
            ...input,
            opId: operationId(this.context.actorId, input.opId),
          }),
        ),
    ...
    const setBudget: TeamToolDefinition = {
      ...
      handler: async (args) =>
        this.mutate(args, budgetInput, (input) =>
  ...
  private async read(args: Record<string, unknown>): Promise<TeamToolHandlerResult> {
    if (Object.keys(args).length !== 0)
      return this.error('Evidence/cost read does not accept scope or other arguments.')
    const budget = this.service.budget()
    const costs = this.service.listCostsPage(100, 0)
    return this.result({

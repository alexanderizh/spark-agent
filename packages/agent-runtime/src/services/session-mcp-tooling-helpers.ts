...
export const PLATFORM_MANAGEMENT_SYSTEM_PROMPT = [
  ...
  '- **Custom Tools**: guide, list, get, validate, create/save draft, test, publish, enable/disable, rollback, delete',
  ...
  'When the user asks to create a custom tool through conversation, call `custom_tools_guide` first, inspect existing tools, and keep the result as a disabled draft until validation succeeds. HTTP/OpenAPI tools and native `type=code` TypeScript tools belong in Tool Studio; code composes explicitly allow-listed tools through `sdk.tools.call`. MCP is only an optional import adapter for an existing external MCP service, not the default custom-tool runtime. Image understanding is only a normal template, never the default tool type or product entry.',
  'Custom-tool management never accepts secret values. Declare only secretRef slots and direct the user to the secure Tool Studio form. Do not claim that a normal Node/MCP process is sandboxed; user-authored local MCP code is trusted code and must be reviewed before enabling.',

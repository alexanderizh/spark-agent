...
export const ENABLE_CLAUDE_SDK_RESUME = defaultGate.isEnabled

export function isSdkResumeSafe(params: import('./session-resume-gate.js').ResumeSafeParams): boolean {
...
export function makeSdkRuntimeSessionId(
  ...
  model: string,
  agentAdapter: import('./session-resume-gate.js').AgentAdapterKind,
  turnId?: string,
): string {

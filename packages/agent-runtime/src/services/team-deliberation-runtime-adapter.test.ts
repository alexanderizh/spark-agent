...
describe('TeamDeliberationRuntimeAdapter', () => {
  ...
  beforeEach(() => {
    dir = join(
      tmpdir(),
      `spark-team-deliberation-runtime-${Date.now()}-${Math.random().toString(36).slice(2)}`,
    )
  ...
  it('keeps every deliberation tool schema representable for SDK MCP tools/list', () => {
    const adapter = new TeamDeliberationRuntimeAdapter(db, {
      sessionId: 'session-a',
      discussionId: 'discussion-a',
      actorId: 'user-a',
    ...
    for (const definition of adapter.buildToolDefinitions()) {
      expect(() => z.toJSONSchema(z.object(definition.schema))).not.toThrow()
    }
  })
  ...
  it('supports proposal chain, vote evidence, CAS, idempotency, and ledger callback', async () => {
    ...
    const tool = (name: string) =>
      adapter.buildToolDefinitions().find((definition) => definition.name === name)!
    ...
    const decided = await tool('team_deliberation_decide').handler({
      id: 'proposal-a',
      expectedVersion: voted.version,
      decision: {
        outcome: 'approved',
        reason: 'Ship it',
        ledgerWrite: { logicalKey: 'release', value: { approved: true }, reason: 'decision' },
      },
      opId: 'decide-a',
    })
  ...
  it('rejects unsafe payloads before persistence and maps missing records to errors', async () => {
    ...
    const create = adapter
      ...
      .find((definition) => definition.name === 'team_deliberation_propose')!
    expect(
      (
        await create.handler({
          id: 'bad',
          topic: 'x',
          proposal: { claim: 'x', position: 'support', rationale: 'x' },
          opId: 'x',
          extra: true,
        })
      ).isError,
    ...
    const evidence = adapter
      .buildToolDefinitions()
      .find((definition) => definition.name === 'team_deliberation_add_evidence')!
    const missing = await evidence.handler({
      id: 'missing',
      expectedVersion: 1,
      evidence: { summary: 'x', sourceRef: 's', polarity: 'supports' },
      opId: 'missing',
    })

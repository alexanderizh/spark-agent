      ...
      .map((ref) => {
        const normalizedRef: ProviderMediaModelRef = {
          ...
          ...(ref.displayName != null && ref.displayName.trim().length > 0
            ...
            : {}),
          // adapterMode 决定 ref 是否强制走模板 adapter，capabilityOverrides 覆盖
          // capability 级 defaults/aliases/paramPolicy；两者被 resolver 与 router 消费，
          // 归一化时丢弃会让模板路由和按模型参数覆盖在保存后静默失效。
          ...(ref.adapterMode != null ? { adapterMode: ref.adapterMode } : {}),
          ...(ref.capabilityOverrides != null
            ? { capabilityOverrides: ref.capabilityOverrides }
            : {}),

...
/** OpenAI Fast mode is a per-session opt-in stored alongside other metadata feature flags. */
export function getFastModeFromMetadata(metadataJson: string | null | undefined): boolean {
  if (metadataJson == null || metadataJson.length === 0) return false
  try {
    return (JSON.parse(metadataJson) as { fastMode?: unknown }).fastMode === true
  } catch {
    return false
  }
}
...
/**
 * 判断排队 turn 的 runtimePatch 是否只携带 Composer 的完整运行时快照（文本注入等价）。
 * - 返回 false：patch 含 skill，或 Agent/权限/模式等字段相对当前会话确有变化；
 * - 返回 null：可以内联，且没有需要持久化的运行时选择；
 * - 返回对象：可以内联，并把 provider/model/CLI 选择及其余未变化快照持久化。
 * 渲染端每条消息都携带当前 provider/model/agent/adapter/权限/模式/推理档位，
 * 若把「带 patch」一律视为不可排空，
 * goal 插话注入将永远无法触发。
 */
export function pickGoalDrainableRuntimeSelection(
  patch: SessionRuntimePatch | undefined,
  currentRuntime: SessionRuntimePatch,
): SessionRuntimePatch | null | false {
  if (patch == null) return null
  const drainableKeys = new Set<keyof SessionRuntimePatch>([
    'providerProfileId',
    'modelId',
    'cliSparkOverride',
    'agentId',
  ...
  const isRuntimeOnly = Object.keys(patch).every((key) =>
    drainableKeys.has(key as keyof SessionRuntimePatch),
  )
  if (!isRuntimeOnly) return false
  const stableSnapshotKeys: ReadonlyArray<keyof SessionRuntimePatch> = [
    'agentId',
    'agentAdapter',
    'permissionMode',
    'chatMode',
  ...
  const changesNonSelectionRuntime = stableSnapshotKeys.some(
    (key) => patch[key] !== undefined && patch[key] !== currentRuntime[key],
  )
  if (changesNonSelectionRuntime) return false
  return getRuntimePatch(patch) ?? null
}
...
export function getRuntimePatch(params: SessionRuntimePatch): SessionRuntimePatch | undefined {
  ...
  if (params.cliSparkOverride !== undefined) {
    patch.cliSparkOverride = normalizeCliSparkOverride(params.cliSparkOverride)
  }
  ...
  if (params.fastMode !== undefined) patch.fastMode = params.fastMode
  return Object.keys(patch).length > 0 ? patch : undefined
}

/**
 * Fast mode is an OpenAI-protocol request feature. Third-party compatible providers may reject
 * the parameter; preserving that upstream error is intentional, so support is protocol-based.
 */
export function supportsOpenAIFastMode(params: {
  isLocalCli: boolean
  isLocalCodexCli?: boolean
  hasCliSparkOverride?: boolean
  providerType: string
  codexApiKind?: 'chat' | 'responses' | 'embedding'
}): boolean {
  const hasOpenAITransport =
    !params.isLocalCli || params.isLocalCodexCli === true || params.hasCliSparkOverride === true
  return (

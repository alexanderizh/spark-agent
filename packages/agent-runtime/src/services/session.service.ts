...
import {
  ...
  captureGoalDrainableRuntimeBaseline,
  captureTurnRuntimeSelectionSnapshot,
  type TurnRuntimeSelectionSnapshot,
} from './session/session-runtime-patch.js'
...
export class SessionService {
  ...
  async executeCommandAsEvents(
    ...
    if (
      ...
      const commandRuntimePatch = getRuntimePatch(params)
      const commandTurn: PendingTurn = {
        turnId: crypto.randomUUID(),
        message: params.message,
        enqueuedAt: new Date().toISOString(),
        isCommandTurn: true,
        ...(commandRuntimePatch != null ? { runtimePatch: commandRuntimePatch } : {}),
    ...
    applySessionRuntimePatch(new SessionRepository(this.db), params.sessionId, runtimePatch)
    return this.commandController.executeCommandAsEvents({
      sessionId: params.sessionId,
      message: params.message,
      ...(params.attachments != null ? { attachments: params.attachments } : {}),
      ...(params.sessionReferences != null ? { sessionReferences: params.sessionReferences } : {}),
  ...
  private async startTurnExecution(
    ...
    applySessionRuntimePatch(sessionRepo, sessionId, runtimePatch)

    const session = sessionRepo.findByIdOrFail(sessionId)
    const turnRuntimeSelection = captureTurnRuntimeSelectionSnapshot(session)
    ...
    const effectiveFastMode =
      ...
      supportsOpenAIFastMode({
        isLocalCli,
        isLocalCodexCli: isLocalCodexCliProvider(cliProvider),
        hasCliSparkOverride: activeCliSparkOverride != null,
      ...
      if (hasDispatchableTeamMembers || hasWorkflowExecutionPlan) {
        ...
        teamMcpServer =
          (await this.createTeamMcpServer({
            ...
            workspaceRootPath,
            eventRepo,
            runtimeSelectionSnapshot: turnRuntimeSelection,
  ...
  private async createTeamMcpServer(ctx: {
    ...
    workspaceRootPath: string
    eventRepo: EventRepository
    /** Host turn 起跑时冻结的会话运行时选择，供延迟/嵌套成员继承。 */
    runtimeSelectionSnapshot: TurnRuntimeSelectionSnapshot
        ...
        task,
        {
          ...
          executeMember: ({
            ...
            this.executeMemberTurn({
              ...
              runtimeSelectionSnapshot: ctx.runtimeSelectionSnapshot,
              signal,
              memberDepth,
              deadlineAt,
              members: ctx.members,
  ...
  private async executeMemberTurn(args: {
    ...
    const providerRepo = new ProviderProfileRepository(this.db)
    const session = sessionRepo.findByIdOrFail(sessionId)
    let providerProfileId = member.providerProfileId ?? runtimeSelectionSnapshot.providerProfileId
    ...
    const cliProvider = provider
    const activeCliSparkOverride = isLocalCli ? runtimeSelectionSnapshot.cliSparkOverride : null
    ...
    const effectiveMemberFastMode =
      isCodexMember &&
      getFastModeFromMetadata(session.metadata_json) &&
      supportsOpenAIFastMode({
        isLocalCli,
        isLocalCodexCli: isLocalCodexCliProvider(cliProvider),
        hasCliSparkOverride: appliedCliSparkOverride != null,
        providerType: provider.provider_type,
  ...
  private async executeQueuedCommandTurn(sessionId: string, turn: PendingTurn): Promise<void> {
    applySessionRuntimePatch(new SessionRepository(this.db), sessionId, turn.runtimePatch)
    const result = await this.commandController.executeCommandAsEvents({
      sessionId,
      message: turn.message,
      ...(turn.runtimePatch != null ? { runtimePatch: turn.runtimePatch } : {}),
   ...
   *
   * 只内联「纯文本用户消息」：不带附件/skill/会话参考/mention 的 turn。Composer
   * 每条消息携带完整运行时快照；仅当 Agent/adapter/权限/模式/推理档位与当前
   * 会话相同，才仍视为纯文本等价。排空时允许最后一条 provider/model/CLI 选择
   * 对下一轮迭代生效；stable 字段确有变化或含 skillIds 时保留到目标结束后执行。
  ...
  private drainQueuedUserTurnsForGoalIteration(sessionId: string): string[] {
    ...
    const eventRepo = new EventRepository(this.db)
    const sessionRepo = new SessionRepository(this.db)
    const currentSession = sessionRepo.get(sessionId)
    const currentRuntime: SessionRuntimePatch =
      currentSession == null ? {} : captureGoalDrainableRuntimeBaseline(currentSession)
    const remaining: PendingTurn[] = []
    const drainedMessages: string[] = []
    let drainedCount = 0
    let latestRuntimeSelection: SessionRuntimePatch | null = null
    for (const turn of queue) {
      const runtimeOnlyPatch = pickGoalDrainableRuntimeSelection(turn.runtimePatch, currentRuntime)
    ...
    // 用户在插话里切换了 provider/model/CLI：以最后一条安全快照为准写入会话运行时，
    // 让紧随其后的迭代 turn（goalSyntheticTurnRuntimePatch 读会话快照）真正用上新选择。
    if (latestRuntimeSelection != null) {
      applySessionRuntimePatch(sessionRepo, sessionId, latestRuntimeSelection)
    }
    this.emitQueueChanged(sessionId)
  ...
  private goalSyntheticTurnRuntimePatch(sessionId: string): SessionRuntimePatch | undefined {
    ...
    patch.cliSparkOverride = getCliSparkOverrideFromMetadata(session.metadata_json)

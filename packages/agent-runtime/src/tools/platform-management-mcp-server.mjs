...
const CUSTOM_MEDIA_MODEL_SCHEMA = {
  type: 'object',
  additionalProperties: false,
  required: ['modelId', 'manifest'],
  properties: {
    modelId: { type: 'string', minLength: 1, maxLength: 200, description: '渠道真实模型 ID' },
    displayName: { type: 'string', minLength: 1, maxLength: 200, description: '模型显示名称' },
    enabled: { type: 'boolean', description: '是否启用该模型，默认 true' },
    defaults: {
      type: 'object',
      description: '该模型的 Provider 级默认参数',
...
const CUSTOM_MEDIA_PROVIDER_VALIDATION_PROPERTIES = {
  providerId: {
    type: 'string',
    description: '更新已有 Provider 时传入；省略则创建新 Provider',
  },
  name: { type: 'string', minLength: 1, maxLength: 200, description: '渠道名称；新建时必填' },
  providerProtocol: {
    type: 'string',
    enum: ['openai', 'anthropic'],
    description: '文本兼容协议标识；纯多媒体渠道通常使用 openai，真实媒体鉴权由 manifest 决定',
  },
  apiEndpoint: {
    type: 'string',
    minLength: 1,
    maxLength: 1000,
    description: '渠道 API Base URL，例如 https://api.example.com/v1',
  },
  defaultModel: { type: 'string', minLength: 1, maxLength: 200, description: '默认模型 ID' },
  models: {
    type: 'array',
    minItems: 1,
    maxItems: 100,
    items: CUSTOM_MEDIA_MODEL_SCHEMA,
    description: '模型及其完整适配器合同；同一 Provider 内 modelId 不得重复',
  },
}

const CUSTOM_MEDIA_PROVIDER_DRAFT_PROPERTIES = {
  ...CUSTOM_MEDIA_PROVIDER_VALIDATION_PROPERTIES,
  apiKey: {
    type: 'string',
    description: '可选 API Key。仅在用户主动提供并同意保存时传入；不会出现在返回值或日志中',
  },
  ...
  return [
    ...
    },
    {
      ...
      inputSchema: {
        ...
        additionalProperties: false,
        properties: {
          modelId: { type: 'string', description: '可选真实模型 ID，用于生成起始合同' },
          domain: { type: 'string', enum: ['image', 'video', 'audio'], description: '媒体类型' },
          mode: {
            type: 'string',
    ...
    },
    {
      ...
      inputSchema: {
        type: 'object',
        additionalProperties: false,
        required: ['apiEndpoint', 'defaultModel', 'models'],
        properties: CUSTOM_MEDIA_PROVIDER_VALIDATION_PROPERTIES,
      },
    },
    {
      ...
      inputSchema: {
        ...
        additionalProperties: false,
        required: ['apiEndpoint', 'defaultModel', 'models'],
        properties: CUSTOM_MEDIA_PROVIDER_DRAFT_PROPERTIES,
      },
    },
    {
      name: 'providers_media_discover_models',
      description:
        '调用渠道 /models（或指定 modelsUrl）获取真实模型清单，可用于初始化自定义渠道。可读取已保存 Provider 的 Key，也可校验尚未保存的草稿；不会保存模型。',
      inputSchema: {
        type: 'object',
        additionalProperties: false,
        properties: {
          providerId: { type: 'string', description: '已保存 Provider ID；与草稿参数二选一' },
          providerProtocol: { type: 'string', enum: ['openai', 'anthropic'] },
          apiEndpoint: { type: 'string', description: '草稿 API Base URL' },
          apiKey: { type: 'string', description: '草稿 API Key；不会回显或写日志' },
          modelsUrl: { type: 'string', description: '可选模型地址，相对路径或完整 URL' },
          isFullUrl: { type: 'boolean', description: 'modelsUrl 是否为完整 URL' },
        },
      },
    },
    {
      name: 'providers_media_diagnose',
      description:
        '分阶段诊断已保存的自定义多媒体 Provider：合同校验、Keychain 凭据、/models 和真实生成调用。默认只读；execute 会发起可能计费的真实请求，必须先获得用户明确同意并设置 confirmExecute=true。错误和请求摘要会自动脱敏，适合排查 401、404、参数 400、轮询和产物解析问题。',
      inputSchema: {
        type: 'object',
        additionalProperties: false,
        required: ['providerId'],
        properties: {
          providerId: { type: 'string', description: '待诊断 Provider ID' },
          checkModels: { type: 'boolean', description: '是否调用 /models' },
          modelsUrl: { type: 'string', description: '可选模型地址' },
          isFullModelsUrl: { type: 'boolean', description: 'modelsUrl 是否为完整 URL' },
          execute: {
            type: 'object',
            additionalProperties: false,
            required: ['modelId', 'capabilityId', 'confirmExecute'],
            properties: {
              modelId: { type: 'string', description: '真实调用模型 ID' },
              capabilityId: {
                type: 'string',
                enum: [
                  'image.generate',
              ...
              prompt: { type: 'string', description: '最小诊断提示词' },
              ...
              inputFiles: {
                ...
                items: {
                  ...
                  properties: {
                    ...
                    role: {
                      type: 'string',
                      enum: ['input', 'first_frame', 'last_frame', 'reference', 'mask'],
                    },
                    path: { type: 'string' },
                    url: { type: 'string' },
                    dataUrl: { type: 'string' },
              ...
              confirmExecute: {
                type: 'boolean',
                description: '只有用户明确同意可能计费的真实调用后才能设为 true',
              },
            },
          },
        },
...
async function handleToolCall(name, args) {
  ...
  const methodMap = {
    ...
    providers_media_guide: 'providers.media_guide',
    providers_media_validate: 'providers.media_validate',
    providers_media_configure: 'providers.media_configure',
    providers_media_discover_models: 'providers.media_discover_models',
    providers_media_diagnose: 'providers.media_diagnose',

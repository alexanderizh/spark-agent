...
async function call(method, path, body) {
  ...
  if (json?.ok === false) {
    ...
    if (err && typeof err === 'object')
      throw new Error(`${err.code || 'BROWSER_ERROR'}: ${err.message || 'browser bridge error'}`)
...
const WINDOW_ID_PROP = {
  type: 'string',
  description:
    'Window id returned by open/list_windows. Omit to target the first open spark_browser window.',
}
...
const TOOLS = [
  {
    name: 'open',
    description:
      'Open a visible in-app BrowserWindow and navigate to a URL. Supports http/https/file/data URLs. Omit profileId to use the shared default profile, which keeps the user logins (cookies/localStorage/cache) across turns and app restarts — this is the recommended default so the user stays logged in. Pass a profileId ONLY when the task explicitly requires a clean, isolated logged-out environment.',
    inputSchema: {
      type: 'object',
      properties: {
        url: {
          type: 'string',
          description: 'URL to open. Local HTML should use file:///absolute/path.html.',
        },
        show: { type: 'boolean', description: 'Whether to show the window. Defaults to true.' },
        profileId: {
          type: 'string',
          description:
            'Optional. Omit to share the persistent default profile with the user login state (recommended). Each distinct profileId is a separate EMPTY storage partition with no logins; creating a per-task profileId forces the user to log in again every time. Only isolate when the task needs it, and then reuse a stable name per purpose (e.g. clean-test) instead of task-specific names.',
        },
        reuse: {
          type: 'boolean',
          description: 'Reuse an existing window with the same profileId if present.',
        },
      },
      required: ['url'],
    },
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: { windowId: WINDOW_ID_PROP, url: { type: 'string' } },
      required: ['url'],
    },
  },
  {
    name: 'eval',
    description:
      'Execute JavaScript in the page and return the JSON-serializable result. For DOM nodes, cyclic objects, or complex values, stringify inside the code yourself.',
    inputSchema: {
      type: 'object',
      properties: { windowId: WINDOW_ID_PROP, code: { type: 'string' } },
      required: ['code'],
    },
  },
  {
    name: 'inject_script',
    description:
      'Persistently inject JavaScript into a window. The script is re-run after future navigations. Remove it with remove_script or close the window when finished.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    inputSchema: {
      type: 'object',
      properties: { windowId: WINDOW_ID_PROP, scriptId: { type: 'string' } },
      required: ['scriptId'],
    },
  },
  {
    name: 'screenshot',
    description: 'Capture the current window as a PNG dataUrl plus url/title.',
    inputSchema: { type: 'object', properties: { windowId: WINDOW_ID_PROP } },
  },
  {
    name: 'get_url',
    description:
      'Read the current URL of a spark_browser window, including user manual navigation.',
    inputSchema: { type: 'object', properties: { windowId: WINDOW_ID_PROP } },
  },
  {
    name: 'get_title',
    description: 'Read the current page title of a spark_browser window.',
    inputSchema: { type: 'object', properties: { windowId: WINDOW_ID_PROP } },
  },
  {
    name: 'list_windows',
    description:
      'List open spark_browser windows with profile, visibility, url/title, script count, network rule count, and console buffer count.',
    inputSchema: { type: 'object', properties: {} },
  },
  {
    name: 'close',
    description:
      'Close a spark_browser window and clear its scripts, network rules, and event buffers. Profile storage is preserved unless clear_profile is called.',
    inputSchema: { type: 'object', properties: { windowId: WINDOW_ID_PROP } },
  },
  {
    name: 'console_start',
    description: 'Start capturing console messages (log/warn/error) from a window.',
    inputSchema: { type: 'object', properties: { windowId: WINDOW_ID_PROP } },
  },
  {
    name: 'console_events',
    description: 'Read captured console events. Use sinceSeq to poll incrementally.',
    inputSchema: {
      type: 'object',
      properties: { windowId: WINDOW_ID_PROP, sinceSeq: { type: 'number' } },
    },
  },
  {
    name: 'console_clear',
    description: 'Clear captured console events for a window.',
    inputSchema: { type: 'object', properties: { windowId: WINDOW_ID_PROP } },
  },
  {
    name: 'network_set_rules',
    description:
      'Observe and modify window network traffic. Supports record, block, redirect, and set_headers rules. Response-body mock_response is intentionally unsupported in this build and returns NETWORK_RULE_UNSUPPORTED.',
    inputSchema: {
      type: 'object',
      properties: {
        windowId: WINDOW_ID_PROP,
        rules: {
          ...
          items: {
            ...
            properties: {
              ...
              action: {
                type: 'string',
                enum: ['record', 'block', 'redirect', 'set_headers', 'mock_response'],
              },
  ...
  },
  {
    name: 'network_events',
    description:
      'Read recorded request/completion/error events for a window. Use sinceSeq to poll incrementally.',
    inputSchema: {
      type: 'object',
      properties: { windowId: WINDOW_ID_PROP, sinceSeq: { type: 'number' } },
    },
  ...
  {
    name: 'network_clear',
    description: 'Clear network rules and events, or only the supplied ruleIds.',
    inputSchema: {
      type: 'object',
      properties: {
        windowId: WINDOW_ID_PROP,
  ...
  },
  {
    name: 'clear_profile',
    description:
      'Clear persistent spark_browser profile data such as cookies, cache, localStorage, and IndexedDB. This signs pages out for that profile. Destructive: clearing the default profile erases ALL saved user logins — confirm with the user first.',
    inputSchema: {
      type: 'object',
      properties: {
        profileId: {
          type: 'string',
          description:
            'Profile to clear. "default" holds the user shared login state; clearing it signs the user out everywhere.',
        },
        scope: {
          type: 'array',
          items: { type: 'string', enum: ['cookies', 'cache', 'localStorage', 'indexedDB', 'all'] },
        },
      },
      required: ['profileId'],
    },
...
function summarize(name, data) {
  if (name === 'screenshot')
    return `Screenshot captured for ${data.url || 'window'} (${data.title || 'untitled'}). dataUrl length=${data.dataUrl?.length || 0}`
  ...
  try {
    if (request.method === 'initialize') {
      result(id, {
        ...
        capabilities: { tools: {} },
        serverInfo: { name: 'spark-browser', version: '0.1.0' },
      })
      return
    }

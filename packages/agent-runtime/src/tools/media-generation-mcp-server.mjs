...
// Contract V2 multipart parts 编译：与 TS 侧 compileInvocationRequest 的 multipart
// 分支同语义 —— file part 解析为可上传 Buffer（data:/远程 URL/本地路径），text/json
// part 渲染为 form 字段。legacy manifest（无 request 合同）对齐
// legacyInvocationRequest：requestTemplate 每项均为 text part。
async function compileManifestMultipartForm(manifest, variables) {
  const body = manifest.invocation?.request?.body
  const parts =
    body?.kind === 'multipart' && Array.isArray(body.parts)
      ? body.parts
      : Object.entries(manifest.invocation?.requestTemplate || {}).map(([name, value]) => ({
          name,
          kind: 'text',
          value,
        }))
  ...
  const files = []
  for (const part of parts) {
    if (!part || typeof part.name !== 'string' || !part.name) continue
    if (part.kind === 'file') {
      const resolved = renderTemplateStringOrValue(part.value, variables)
      const values = Array.isArray(resolved) ? resolved : [resolved]
      let index = 0
      for (const value of values) {
        index += 1
        if (value === undefined || value === null || value === '') continue
        const upload = await resolveMcpInputBuffer(value, `${part.name}-${index}`)
        files.push({ field: part.name, ...upload })
      }
      continue
    }
    const resolved = renderTemplateStringOrValue(part.value, variables)
    if (resolved === undefined || resolved === null || resolved === '') continue
    fields[part.name] = part.kind === 'json' ? JSON.stringify(resolved) : String(resolved)
  }
  return buildMcpMultipart(fields, files)
}
...
async function handleManifestTool(config, toolName, args, match) {
  ...
  // Contract V2 json 与 multipart 均可编译；其余（binary 等）暂不支持。
  // 不能 return null —— 调度层会以 data.taskId 直接访问返回值，null 会变成 TypeError。
  const invocationContentType = manifest.invocation?.contentType || 'json'
  if (invocationContentType !== 'json' && invocationContentType !== 'multipart') {
    throw new Error(
      `Manifest contentType "${invocationContentType}" is not supported by the MCP media tool yet`,
    )
  }
  ...
  // Contract V2 multipart：按 parts 合同编译（无 request 合同时从 legacy
  // requestTemplate 推导全 text part，与 TS 侧 legacyInvocationRequest 同语义）。
  const multipartForm =
    invocationContentType === 'multipart'
      ? await compileManifestMultipartForm(manifest, variables)
      : null
  let requestHeaders
  ...
  if (!multipartForm) {
    ...
    serializedRequestBody = multipartForm.body
  }
  if (
    manifest.providerKind === 'openai-images' &&

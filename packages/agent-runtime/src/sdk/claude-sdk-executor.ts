...
export class ClaudeSDKExecutor implements PermissionModeAwareExecutor, RewindCapableExecutor {
  ...
  /**
   * 本 executor 生命周期（= 一个 turn 的一次 SDK query）内直读的图片数。
   * Read 图片会把整张图作为 base64 驻留上下文并在此后每轮重发，超阈值后由
   * canUseTool 硬闸 deny 并引导走 Task 子代理，防止模型无视多图指引连读截图
   * 或在 compact 后循环重读打爆上下文。
   */
  private directImageReads = 0
          ...
          ? {
              canUseTool: async (
                ...
                try {
                  ...
                  // 图片直读硬闸：Read 图片会把整张图 base64 驻留上下文并在此后每轮
                  // 重发，超阈值后 deny 并引导 Task 子代理看图回传文本结论（子代理
                  // 上下文随任务销毁）。必须放在 callbackAllowedTools 放行之前——
                  // Read 属 bare allow，放到后面就拦不到了。
                  if (toolName === 'Read' && isImageFileInput(input)) {
                    this.directImageReads += 1
                    if (this.directImageReads > MAX_DIRECT_IMAGE_READS_PER_TURN) {
                      return denyTool(
                        `Direct image Reads are capped at ${MAX_DIRECT_IMAGE_READS_PER_TURN} per turn: raw image payloads persist in the conversation context and are re-sent on every request. Dispatch a Task subagent to Read the remaining images and report its findings as text, or continue from the findings already recorded.`,
                        callbackOptions.toolUseID,
                      )
                    }
                  }
...
export function buildPromptWithAttachments(
  ...
  // 多图走子代理隔离：图片 Read 会把整张图作为 base64 永久驻留本会话上下文并随
  // 后每轮请求重发，compact 只保留文本摘要、图片丢失后 agent 又倾向重读，几轮就
  // 会把上下文打爆（触发 autocompact thrashing）。少量图片保留直读体验，批量图片
  // 必须引导 Task 子代理看图、以文本结论回传主对话。阈值与 canUseTool 图片硬闸
  // （MAX_DIRECT_IMAGE_READS_PER_TURN）保持一致。
  const imageCount = attachments.filter((attachment) => attachment.type === 'image').length
  const imageInstructions =
    imageCount >= MAX_DIRECT_IMAGE_READS_PER_TURN
      ? [
          `${imageCount} images are attached. Raw image payloads persist in this conversation's context for the rest of the session and each one is re-sent on every request, which quickly exhausts the context window.`,
          'Analyze the images through Task subagents instead: dispatch a subagent to Read the images and report its findings back as text, then work from that text. Do NOT Read the images directly in this conversation.',
          'After a context compaction, do NOT re-Read images that were already analyzed — rely on the recorded findings and the attachment paths listed above.',
        ]
...
/**
 * 主对话单 turn 允许直读（Read）的图片上限。图片以 base64 驻留上下文、每轮重发，
 * 且 compact 只留文本摘要导致 agent 倾向重读，多图直读几轮就会触发 autocompact
 * thrashing。超出部分强制走 Task 子代理（子代理上下文随任务销毁，主对话只收
 * 文本结论）。阈值与 buildPromptWithAttachments 的多图指引保持一致。
 */
const MAX_DIRECT_IMAGE_READS_PER_TURN = 3
...
/** Read 的输入是否指向位图文件（svg 是文本，Read 无负担，不算）。 */
export function isImageFileInput(input: Record<string, unknown>): boolean {

...
export function buildComputerUseSystemPrompt(capabilities: ComputerUsePromptCapabilities): string {
  ...
  return [
    ...
    'A named-app task remains bound to that target when the user focuses or works in another application. Do not stop, restart, or rebind merely because the foreground application changed; the governed host restores the target only for an action and then restores the user foreground app.',
    ...
    'You decide when the task is complete — there is no automatic verifier. Confirm the requested outcome is actually visible in the latest screenshot / app state before declaring success; when done, call `stop` to release desktop control.',
    'A task is never auto-canceled. It keeps running until you call `stop` or the user presses the kill switch, so keep making progress and never abandon a task midway just because one step was slow or imperfect.',
    'If a single action fails, do not give up and do not declare the whole task failed — switch approach: pick a different element, fall back to screen coordinates, use keyboard navigation, or re-read the app state with `get_app_state`. The failure is reported back to you on the next turn precisely so you can choose an alternative next step.',
    'Prefer semantic actions over coordinates: when the latest observation exposes an accessibility element for the target (button, field, link, list item), act on it by element id (`invoke_element`, `set_value`, `select_text`) instead of clicking screen coordinates. Coordinate click/scroll/type_text actions are the second choice, reserved for custom-drawn or tree-less UI where no element matches; semantic actions also run in the background without stealing your focus and are more reliable.',
    '',

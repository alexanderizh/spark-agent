...
describe('buildComputerUseSystemPrompt', () => {
  ...
  it('keeps the delegated start_task path and evidence-backed completion', () => {
    ...
    expect(prompt).toContain('pressing Esc anywhere')
    expect(prompt).toContain('do NOT retry the action')
    expect(prompt).toContain('synthesized keys are not user input')

...
| 任务 | 内容 | 依赖 |
|------|------|------|
| T1 骨架与纯函数层 | 1.1~1.4 + 2.1 + 5 + 对应单测 | 无 |
| T2 视口姿势 Gizmo | 2.2 + 3.1~3.4 + `poseIk.ts` 单测 | T1 |
| T3 效率功能 | 1.5 + 3.5 + 4.1~4.3 + 对应单测 | T1（与 T2 弱耦合） |

验收标准：`pnpm typecheck` 通过；stage3d 单测全绿；真机冒烟——能在视口中用环+IK 摆出「弓步冲拳」并保存为自定义姿势、镜像、撤销重做各操作一遍。

...
// 发布语音离线精修模型（SenseVoice int8）到自建源。
// 用法: node scripts/publish-voice-refine-to-minio.mjs <version> [artifact-dir]
// artifact-dir 内需存在:
//   - voice-refine-<version>-manifest.json  (release entry, 见 validateReleaseEntry)
//   - <entry.url 的 basename>               (tar.gz 包, sha256/size 与 entry 一致)
...
const artifactDirectory = resolve(
  process.argv[3] || `/private/tmp/spark-voice-refine-${version || 'unknown'}`,
)
...
if (!version || !/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(version)) {
  fail('Usage: node scripts/publish-voice-refine-to-minio.mjs <version> [artifact-dir]')
}
...
const releaseManifestPath = join(artifactDirectory, `${ARCHIVE_BASENAME}-${version}-manifest.json`)
...
if (existing && stableJson(existing) !== stableJson(releaseEntry)) {
  fail(
    `manifest contains a conflicting artifact: ${releaseEntry.id} (bump version or align the entry)`,
  )
}
...
const stagingKey = `${repositoryPrefix}/staging/index-voice-refine-${version}-${Date.now()}.json`
...
const published = publishedManifest.artifacts?.find((candidate) => candidate.id === releaseEntry.id)
...
if (publicArtifact.size !== releaseEntry.size || publicArtifact.sha256 !== releaseEntry.sha256) {
  fail(`public artifact mismatch: size=${publicArtifact.size}, sha256=${publicArtifact.sha256}`)
}
...
function validateReleaseEntry(entry) {
  if (
    ...
    !entry.url.endsWith(`${ARCHIVE_BASENAME}-${version}.tar.gz`) ||
    !/^[0-9a-f]{64}$/.test(entry.sha256 ?? '') ||
    !Number.isSafeInteger(entry.size) ||
    entry.size <= 0 ||
    entry.archive?.format !== 'tar.gz'
  ) {
    fail(`invalid voice refine release manifest: ${releaseManifestPath}`)
  }
}

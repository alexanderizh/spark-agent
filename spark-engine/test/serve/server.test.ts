...
async function start(script: readonly FakeScriptItem[]): Promise<{
  ...
  const handle = await startServeServer({ agent, engineVersion: '0.0.0-test', port: 0 })
  handles.push(handle)
  return { handle, baseUrl: `http://127.0.0.1:${handle.handshake.port}`, agent }
}
...
describe('spark serve protocol v1', () => {
  ...
  it('cancels the active turn and reports 409 when nothing runs', async () => {
    ...
    const agent = Agent.open({ cwd: root, env: { ...base, llm: hanging } })

...
function terminateTree(pid: number | undefined, signal: NodeJS.Signals): void {
  ...
  try {
    ...
    // ESRCH means the process/group already exited. EPERM can occur on POSIX
    // when a very short-lived detached group disappears or its id is reused
    // between the output event and cleanup. Neither condition should escape a
    // stream callback as an uncaught exception; do not retry against a pid that
    // may now identify an unrelated process.
    if (!isIgnorableTerminationError(error)) throw error
  }
}

function isIgnorableTerminationError(error: unknown): boolean {
  return (
    typeof error === 'object' &&
    error !== null &&
    'code' in error &&
    ['ESRCH', 'EPERM'].includes(String((error as { readonly code?: unknown }).code))
  )
}

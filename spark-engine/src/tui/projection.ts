...
export function projectTranscript(
  ...
  for (const candidate of events) {
    ...
    switch (event.type) {
      case 'turn.started':
        settled.push({ key: `event-${event.seq}`, text: `${symbols.user} ${event.input.text}`, tone: 'accent' });
        break;
      ...
      case 'tool.result': {
        ...
        settled.push({
          ...
          text: `${symbols.bar} ${call?.tool ?? 'unknown'} ${preview(call?.args)} ${mark} ${event.durationMs}ms`,
      ...
      case 'turn.completed':
        settled.push({
          key: `event-${event.seq}`,
          text: `${symbols.divider.repeat(2)} 完成 · ${event.stats.steps} steps · ${event.stats.usage.inputTokens + event.stats.usage.outputTokens} tok · $${event.stats.costUsd.toFixed(3)} ${symbols.divider.repeat(2)}`,
          tone: 'dim',
        });
        break;
      case 'turn.cancelled':
      ...
      case 'plugin.deactivated':
        settled.push({ key: `event-${event.seq}`, text: `[${event.type}] ${event.pluginId}`, tone: 'dim' });
        break;
      case 'user.answered':
        settled.push({ key: `event-${event.seq}`, text: `回答: ${preview(event.answer)}`, tone: 'dim' });
        break;

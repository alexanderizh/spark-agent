...
async function* decodeOpenAiEvents(
  ...
  if (!completed) {
    throw new KernelError('llm.incomplete_stream', 'OpenAI stream ended before response.completed', {
      retryable: true,
      detail: { ...(requestId ? { requestId } : {}) },
    });
  }
...
function parseFunctionCall(
  ...
  try {
    ...
    throw new KernelError('llm.openai.invalid_tool_json', 'OpenAI returned invalid function arguments', {
      cause: error,
      detail: { ...(requestId ? { requestId } : {}) },
    });
  }

    ...
    <>
      ...
      <Section title="本地开发" intro="克隆仓库后，你可以分别启动桌面端和官网项目进行开发与验证。">
        <div className="doc-long">
          <h3>环境要求</h3>
          <p>
            Node.js 22+、pnpm、Git；桌面端构建还需各平台原生工具链（Windows 下 Visual Studio Build
            Tools）。
          </p>
          <h3>常用命令</h3>
          <p>
            <code>pnpm install</code> 安装依赖；<code>pnpm dev</code>
            （apps/desktop）启动桌面端；官网在 apps/website 下 <code>pnpm dev</code> 启动本地预览。
          </p>
          <h3>扩展点</h3>
          <p>
            新增模型服务可从 Provider 适配入手；新增工具优先做成 MCP Server 或 Skill；UI
            改动遵循现有样式约定。
          </p>
        </div>
      </Section>

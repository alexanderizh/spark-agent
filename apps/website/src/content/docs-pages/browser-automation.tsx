...
const Body = () => (
  <>
    <p>
      ...
      调试、持久脚本、profile 登录态、console 与网络观察。
    </p>
...
└──────────────────────┘     └─────────────────────────────┘`}
    ...
    <ul>
      <li>
        <strong>Playwright MCP</strong>：适合点击、输入、snapshot/ref、采集和 E2E 验证。
      </li>
      <li>
        <strong>spark_browser</strong>：适合用户可见窗口、本地 <code>file://</code>{' '}
        HTML、持久注入、profile 登录态、console/network 调试。
      </li>
    ...
    <p>
      正常安装后，两套 MCP 都会默认加载：managed <code>playwright</code> 自动注册并启用，
      本地会话自动注入 <code>spark_browser</code>。以下操作只用于完整性修复或切换运行模式：
    </p>
    <p>
      进入 <strong>设置 → 浏览器自动化</strong>：
    </p>
    <ol>
      <li>
        检查或重新安装 <code>@playwright/mcp</code>。
      </li>
      <li>系统没有 Chrome/Edge 时下载 Playwright 使用的 Chromium。</li>
      <li>
        确认 managed <code>playwright</code> MCP 未被手动禁用。
      </li>
    ...
    <table>
      ...
      <tbody>
        <tr>
          <td>可靠点击、输入、表单、snapshot/ref</td>
          <td>
            <code>playwright</code>
          </td>
        </tr>
        <tr>
          <td>网页采集、下载、E2E 验证</td>
          <td>
            <code>playwright</code>
          </td>
        </tr>
        <tr>
          <td>用户可见同一窗口、本地 HTML 调试</td>
          <td>
            <code>spark_browser</code>
          </td>
        </tr>
        <tr>
          <td>持久脚本、console、network、profile 登录态</td>
          <td>
            <code>spark_browser</code>
          </td>
        </tr>
    ...
    <ul>
      <li>
        <code>open</code> / <code>navigate</code> / <code>close</code>：管理可见窗口。
      </li>
      <li>
        <code>eval</code>：执行一次 JS，返回可 JSON 序列化结果。
      </li>
      <li>
        <code>inject_script</code> / <code>remove_script</code>：跨导航持久注入和清理。
      </li>
      <li>
        <code>console_start</code> / <code>console_events</code>：捕获页面日志、警告、错误。
      </li>
      <li>
        <code>network_set_rules</code> / <code>network_events</code>
        ：记录、阻断、重定向、修改请求头。
      </li>
      <li>
        <code>clear_profile</code>：清理指定 profile 的 cookies/cache/localStorage/IndexedDB。
      </li>
    </ul>

    <h2 id="examples">5. 使用示例</h2>
    <p>
      <strong>网页信息采集</strong>
    </p>
    <pre>{`请打开 https://news.ycombinator.com，把首页前 10 条新闻整理成 Markdown 表格。`}</pre>
    <p>优先用 Playwright 的 navigate / snapshot / click / evaluate。</p>
    <p>
      <strong>本地 HTML 调试</strong>
    </p>
    ...
    <table>
      ...
      <tbody>
        <tr>
          <td>Playwright 工具未出现</td>
          <td>设置 → 浏览器自动化：确认 MCP 已启用；重启会话。</td>
        </tr>
        <tr>
          <td>
            <code>browser_navigate</code> 启动失败
          </td>
          <td>下载 Chromium，或确认系统 Chrome/Edge 可用。</td>
        </tr>
        <tr>
          <td>
            <code>spark_browser</code> 没有打开窗口
          </td>
          <td>确认当前是桌面本机会话；远程会话需开启「使用内置浏览器窗口」。</td>
        </tr>
        <tr>
          <td>
            <code>mock_response</code> 不可用
          </td>
          <td>当前仅支持 record/block/redirect/set_headers，响应体 mock 会返回 unsupported。</td>
        </tr>
      </tbody>
    </table>
    ...
    <ul>
      <li>
        <code>spark_browser</code> 页面保持 sandbox、无 Node、无 Electron IPC。
      </li>
      <li>
        profile 会保留登录态，按需用 <code>clear_profile</code> 清理。
      </li>
...
export const browserAutomation: DocsPageContent = {
  ...
  faq: [
    ...
    },
    {
      ...
      answer:
        '不能。旧视图已经移除，Playwright 会自启动浏览器，应用内可见窗口由 spark_browser 提供。',
    },
    {
      question: 'spark_browser 会给网页 Node 权限吗？',
      answer:
        '不会。页面保持 sandbox，Agent 通过主进程受控工具执行 eval、截图、console 和网络观察。',
    },
  ],
  quickReference: [
    ...
    { key: 'Playwright 工具', value: 'mcp__playwright__browser_*' },
    {
      ...
      value: 'open / eval / inject_script / console_events / network_events',
    },

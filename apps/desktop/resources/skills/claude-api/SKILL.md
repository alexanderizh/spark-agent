---
name: claude-api
description: 'Build, debug, and optimize Claude API / Anthropic SDK apps. Apps built with this skill should include prompt caching. Also handles migrating existing Claude API code to current Claude 5 models or replacing retired models. TRIGGER when: code imports `anthropic`/`@anthropic-ai/sdk`; user asks for the Claude API, Anthropic SDK, or Managed Agents; user adds/modifies/tunes a Claude feature (caching, thinking, compaction, tool use, batch, files, citations, memory) or model (Fable/Opus/Sonnet/Haiku) in a file; questions about prompt caching / cache hit rate in an Anthropic SDK project. SKIP: file imports `openai`/other-provider SDK, filename like `*-openai.py`/`*-generic.py`, provider-neutral code, general programming/ML.'
...
## Current Models (verified: 2026-07-30)
...
If the user has not chosen a model, start with `claude-opus-5` for complex agentic and enterprise work. Use `claude-fable-5` when the user explicitly prioritizes the highest available capability, `claude-sonnet-5` for a speed/intelligence balance, and Haiku only when low latency or cost is an explicit requirement.
...
**Claude 5:** Use adaptive thinking. Fable 5 always uses adaptive thinking; Opus 5 and Sonnet 5 support it. Do not introduce fixed `budget_tokens` into new Claude 5 code. Query the Models API before relying on a model-specific effort level or sampling parameter.
**Effort parameter:** Configure effort under `output_config`. Claude Opus 5 and Sonnet 5 default to `high` on the Claude API. Set it explicitly when cost, latency, or depth matters, and verify supported values through the Models API.

**Thinking display:** Do not assume reasoning text will be present. Handle empty/omitted thinking blocks and use documented summary-display controls only after checking the live model capability.

**Task Budgets:** This is a beta feature and may change. Check the live release notes and Models API before generating code that depends on it.

**Sonnet 5:** Supports adaptive thinking; prefer it over fixed thinking budgets.

**Older models (only if explicitly requested):** Consult `shared/model-migration.md` and the live deprecation page. Never infer an old model's thinking schema from Claude 5.

---

## Compaction (Quick Reference)

For long-running conversations, check the Models API for compaction support and follow the live compaction documentation. Preserve all returned content blocks in conversation history.
...
**Migrating to a newer model (Claude 5 family or a supported Claude 4 model) or replacing a retired model:**
...
- **Claude 5 thinking:** Query the Models API for the selected model's live capabilities. Use adaptive thinking when supported; Claude Fable 5 keeps adaptive thinking enabled.
...
- **Confirm migration scope before editing:** When a user asks to migrate code to a newer Claude model without naming a specific file, directory, or file list, ask which scope to apply first. Proceed without asking only when the prompt names an exact file, directory, or explicit file list. See `shared/model-migration.md` Step 0.
...
- **Large outputs:** Current Claude 5 models support up to 128K `max_tokens`; query the Models API before relying on a cached limit. Stream large outputs to avoid SDK HTTP timeouts, using `.stream()` with `.get_final_message()` / `.finalMessage()`.

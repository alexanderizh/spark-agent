...
import {
  EvidenceCostConflictError,
  EvidenceCostService,
} from '../../../../../packages/storage/src/evidence-cost.service.js'
import type {
  CostEvent,
  EvidenceCostSnapshot,
  EvidenceRecord,
} from '../../../../../packages/protocol/src/evidence-cost.js'
...
export class TeamEvidenceCostBackend {
  ...
  private readonly discussions: Pick<
  ...
  getSnapshot(sessionId: string, expectedDiscussionId: string): EvidenceCostSnapshot {
    ...
    if (discussion == null || discussion.id !== expectedDiscussionId)
  ...
  mutate(request: EvidenceCostMutation): EvidenceCostSnapshot {
    ...
    if (discussion == null || discussion.id !== request.expectedDiscussionId)
      throw new SparkError('CONFLICT', '团队讨论已切换，请刷新后重试。')
    const service = EvidenceCostService.forUser(
    ...
    try {
      if (request.kind === 'evidence') {
        if (request.action === 'add')
          ...
          service.invalidateEvidence({
            id: request.id,
            expectedVersion: request.expectedVersion,
            reason: request.reason ?? '未提供原因',
            opId: request.opId,
          })
      ...
      if (error instanceof EvidenceCostConflictError)
        throw new SparkError('CONFLICT', error.message)
      throw error
    }
  }
  ...
  private snapshot(sessionId: string, discussionId: string): EvidenceCostSnapshot {
    const service = EvidenceCostService.forUser(
      this.dependencies.db,
      this.scope(sessionId, discussionId),
    )
    const budget = service.budget()
    const costs = service.listCostsPage(100, 0)
    return {
      sessionId,
      roomId: `team-room:${sessionId}`,
      discussionId,
      evidence: service.listEvidence(100),
      costs: costs.items,
      costsTotal: costs.total,
      costsHasMore: costs.hasMore,
      aggregates: service.aggregate(),

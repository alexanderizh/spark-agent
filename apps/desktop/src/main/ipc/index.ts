...
} from '../services/CanvasSnapshotRetention.js'
...
import { registerImageProcessIpc } from './registerImageProcessIpc.js'
...
export function registerAllIpcHandlers(): void {
  ...
  typedIpcHandle('browser-panel:window-open', async (req) => {
    return getBrowserPanelWindowService().open(req)
  })

  typedIpcHandle('browser-panel:window-restore-panel', async (req) => {
    return getBrowserPanelWindowService().restoreToPanel(req)
  })

  typedIpcHandle('browser-panel:pick-to-composer', async (req) => {
    return getBrowserPanelWindowService().forwardPickToComposer(req.referenceJson)
  })
  ...
  getCanvasWindowService().onProjectExited((projectId) => {
    void (async () => {
      ...
      const removed = await pruneCanvasSnapshots(
  ...
  typedIpcHandle('session:send-turn', async (req) => {
    ...
    return getSessionService().sendTurn({
      ...
      ...(req.cliSparkOverride !== undefined ? { cliSparkOverride: req.cliSparkOverride } : {}),
      ...(req.skillIds !== undefined ? { skillIds: req.skillIds } : {}),
  ...
  typedIpcHandle('session:clear-queued-turns', async (req) => {
    log.info(`session:clear-queued-turns requested, sessionId=${req.sessionId}`)
  ...
  typedIpcHandle('canvas:media:prune-model-params', async (req) => {
    ...
    // 两个分支都必须回传 canonical 键空间：prune 结果会写回 task.modelParams，
    // 执行期编译器只在 canonical 空间识别字段；若这里返回 provider 原生键名，
    // 不在 CANONICAL_ALIASES_FALLBACK 内的键会被 strict 过滤当未知参数丢掉。
    return {
  ...
  typedIpcHandle('canvas:media:prune-model-params-by-inline-manifest', async (req) => {
    ...
    // 与上方 prune 通道同口径：回传 canonical 键空间，保证写回 modelParams 的键名可被
    // 执行期编译器重新识别（inline manifest 编辑器的 dry-run 预览也依赖这一致性）。
    return {
      prunedModelParams: result.canonicalParams,
      droppedParams: result.droppedParams,
      warnings: result.warnings,
  ...
  typedIpcHandle('command:execute', async (req) => {
    ...
    const cmdResult = await getSessionService().executeCommandAsEvents(req)
  ...
  typedIpcHandle('window:ensure-width', async (req) => {
    ...
    // req.minWidth 是 renderer 侧的 CSS 像素需求；窗口 bounds 是屏幕像素。
    // 界面缩放（zoomFactor ≠ 1）时两者相差 zoom 倍，必须换算后再比较，

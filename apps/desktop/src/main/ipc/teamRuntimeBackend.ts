...
export class TeamRuntimeBackend {
  ...
  constructor(private readonly dependencies: TeamRuntimeBackendDependencies) {
    ...
    this.discussions =
      dependencies.discussionRepository ?? new TeamDiscussionRepository(dependencies.db)
  ...
  getTaskGraph(sessionId: string): TaskGraphSnapshot {
    ...
    if (discussion == null)
      return {
        sessionId: sessionId as TaskGraphSnapshot['sessionId'],
        discussionId: null,
        nodes: [],
        edges: [],
        syncedAt: this.now().toISOString(),
      }
    return TaskGraphService.forUser(
      this.dependencies.db,
      this.scope(sessionId, discussion.id),
    ).snapshot() as TaskGraphSnapshot
  }
  ...
  mutateTaskGraph(request: TaskGraphMutation): { snapshot: TaskGraphSnapshot } {
    ...
    const service = TaskGraphService.forUser(
      this.dependencies.db,
      this.scope(request.sessionId, discussion.id),
    )
    try {
      if (request.kind === 'edge') {
        if (request.action !== 'create')
          throw new SparkError('VALIDATION_FAILED', '不支持的任务边操作。')
        service.createEdge({
          id: request.id,
        ...
        service.transition({
          ...
          ...(request.acceptanceStatus !== undefined
  ...
  getDeliberation(sessionId: string): DeliberationSnapshot | null {
    ...
    return DeliberationService.forUser(
      this.dependencies.db,
      this.scope(sessionId, discussion.id),
    ).snapshot()
  }

  mutateDeliberation(request: DeliberationMutateRequest): {
    record: import('@spark/protocol').DeliberationRecord
    snapshot: DeliberationSnapshot
  } {
    const discussion = this.assertDiscussion(request.sessionId, request.expectedDiscussionId)
    const service = DeliberationService.forUser(
      this.dependencies.db,
      this.scope(request.sessionId, discussion.id),
    )
    const base = {
          ...
          : request.action === 'evidence' || request.action === 'vote'
            ? service.addEvidence({
                ...
                evidence:
                  request.action === 'vote'
                    ? {
                        summary: `Vote: ${request.vote?.position}; ${request.vote?.reason}`,
                        sourceRef:
                          request.vote?.sourceRef ??
                          `vote:${this.scope(request.sessionId, discussion.id).actorId}`,
                        polarity:
                          request.vote?.position === 'oppose'
                            ? 'challenges'
                            : request.vote?.position === 'support'
                              ? 'supports'
                              : 'neutral',
                      }
                    : request.evidence!,
              ...
              : request.action === 'risk'
                ...
                : request.action === 'decide'
                  ...
                  : request.action === 'resolve'
                    ? service.resolve({
                        ...base,
                        conflictingRecordId: request.conflictingRecordId!,
  ...
  private scope(sessionId: string, discussionId: string) {
    return { sessionId, roomId: `team-room:${sessionId}`, discussionId, actorId: 'desktop-user' }
  }
  private resolveDiscussion(sessionId: string) {
    ...
    return (
      this.discussions.findActiveBySession(sessionId) ??
      this.discussions.listBySession(sessionId, 1)[0] ??
      null
    )
  }
  private assertDiscussion(sessionId: string, expected: string) {
    const discussion = this.resolveDiscussion(sessionId)
    if (discussion == null) throw new SparkError('VALIDATION_FAILED', '当前团队会话尚未建立讨论。')
    if (discussion.id !== expected)
...
function mapConflict(error: unknown): unknown {
  return error instanceof Error &&
    /conflict|version|quota|cycle|not found|retry/i.test(error.message)
    ? new SparkError('CONFLICT', error.message)
    : error
}

  ...
  async execute(input: {
    ...
    return this.measure('action_ms', () =>
      this.runControlOperation('execution', input.envelope.action, async (connection) => {
        ...
        // Passive actions (wait_for / observe) execute through no transport and report no
        // channel; leave them out rather than diluting the per-channel success rates.
        if (actionResult.executionChannel != null) {
          this.metrics?.recordExecutionOutcome(
            actionResult.executionChannel,

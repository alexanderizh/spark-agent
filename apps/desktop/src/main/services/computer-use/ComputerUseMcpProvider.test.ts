...
describe('createComputerUseMcpProvider', () => {
  it('mounts spark_computer over authenticated loopback HTTP without ELECTRON_RUN_AS_NODE', async () => {
    ...
    expect(config?.allowedTools).toContain('mcp__spark_computer__list_apps')
    expect(config?.allowedTools).toContain('mcp__spark_computer__list_windows')
    expect(config?.allowedTools).toContain('mcp__spark_computer__get_screen_state')
    expect(config?.allowedTools).toContain('mcp__spark_computer__get_app_state')
    expect(config?.allowedTools).toContain('mcp__spark_computer__open_app')

...
describe('NativeHostComputerUseBackend', () => {
  ...
  it('surfaces the host-reported execution channel and records its outcome', async () => {
    ...
    ).resolves.toMatchObject({ noop: false, executionChannel: 'background_ax' })
    expect(metrics.executionChannelSnapshot()).toEqual({
      total: 1,
      backgroundShare: 1,
      channels: [{ channel: 'background_ax', count: 1, failures: 0, successRate: 1, share: 1 }],
    })
  })

...
const mocks = vi.hoisted(() => ({
  ...
  selectCodexRuntimeArtifact: vi.fn(() => ({
    artifact: { version: '2.0.0', size: 20 },
    reason: 'exact-sdk-match',
    candidateVersions: ['2.0.0'],
  })),
...
vi.mock('../CodexRuntimeIntegrityService.js', () => ({
  ...
  selectCodexRuntimeArtifact: mocks.selectCodexRuntimeArtifact,
  describeCodexRuntimeSelection: vi.fn(() => '运行时不可用'),
}))
...
describe('external capability adapters', () => {
  ...
  it('reports the manifest version as targetVersion when a managed component has an update', async () => {
    await expect(
      getExternalCapabilityAdapter('codex-runtime').describe(context),
    ...
    await expect(
      getExternalCapabilityAdapter('voice-pack').describe(context),
    ).resolves.toMatchObject({
      state: 'update_available',

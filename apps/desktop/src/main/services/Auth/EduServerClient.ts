...
export class EduServerClient {
  ...
  async uploadFile(input: {
    ...
    const doUpload = (token?: string) =>
      this.rawUpload('/upload', input, token, this.uploadTimeoutMs)
  ...
  private async rawUpload(
    ...
    // 渠道中转的临时文件走 ?tmp=1：落服务端临时桶，由 MinIO Lifecycle 定期自动清理；
    // 服务端临时桶未就绪时自动回退主桶上传（文件永久保留），客户端无需感知。
    const useTmp = input.purpose === 'media-transfer'
    const url = useTmp ? `${this.resolveUrl(path)}?tmp=1` : this.resolveUrl(path)

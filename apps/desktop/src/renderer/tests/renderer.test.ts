...
describe('Renderer Smoke Tests', () => {
  ...
  function mockAppContextForChatView() {
    ...
    // ChatView 等组件经 useSubAppSurfaces 消费统一侧面板子应用目录；
    // 测试不渲染真实 Provider（会触发 sub-app 目录 IPC），给空目录控制器。
    vi.doMock('../design/sub-app/SubAppSurfaceHost', () => {
      ...
      return {
        SubAppSurfaceProvider: ({ children }: { children: React.ReactNode }) => children,
        useSubAppSurfaces: () => surfaceController,
      }
    })
  }
  ...
  it('omits blank optional runtime fields when creating a project session', async () => {
    ...
    vi.doMock('../design/AppContext', () => ({
      AppDialogHost: () => null,
      useApp: () => ({
        // SessionSidebarContext 会读 t.view（'chat' 时上报已读会话）
        t: { view: 'chat' },
  ...
  it('shows collapsed completed turns with full final answer and visible deliverable cards', async () => {
    const invoke = vi.fn(async (channel: string) => {
      if (channel === 'workspace:list') {
        return {
          workspaces: [
      ...
      if (channel === 'session:get-history') {
        return {
          events: [
            ...
            {
              id: 'assistant-1-presented-files',
              type: 'presented_files',
              sessionId: 'session-1',
              turnId: 'turn-1',
              timestamp: '2026-05-27T00:00:05.900Z',
              seq: 13,
              files: [{ path: '/tmp/spark-agent/output/demo-video.mp4', title: '演示视频' }],
            },
          ],
          hasMore: false,
        }
    ...
    try {
      ...
      // 折叠态仍须展示交付产物：历史轮的视频卡片不被整轮折叠收起。
      expect(container.querySelector('.presented-media-video')).not.toBeNull()
      expect(container.textContent).toContain('演示视频')
      expect(

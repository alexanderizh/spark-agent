...
type AppCtx = {
  ...
  applySyncedAppearance: (value: Record<string, unknown>) => void
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const applySyncedAppearance = useCallback<AppCtx['applySyncedAppearance']>((value) => {
    const patch = pickVisualTweaks(value)
    if (Object.keys(patch).length === 0) return
    hasUserVisualChangeRef.current = true
    if (patch.theme != null) window.localStorage.setItem(THEME_STORAGE_KEY, patch.theme)
    // AccountSync main has already persisted the complete canonical appearance.
    // Update renderer/localStorage only; another IPC write could race and restore stale fields.
    writeLocalAppearanceRecord({ ...readLocalAppearanceSettings(), ...value })
    setT((prev) => ({ ...prev, ...patch }))
  }, [])

...
export function useCanvasWorkspace(projectId: string) {
  ...
  /**
   * 重新拉取快照并应用（P2 资产库）：容器直接经 canvasApi 写库（收藏 / 批量
   * 删除）后调用，等价于 store 内置方法的「调用 + applyCanvasMutationSnapshot」
   * 收尾，保证 usageCounts / 分类计数等派生数据随之刷新。
   */
  ...
  /**
   * 批量删除资产（assetLibrary 缺陷 4）：单次遍历移除 + 级联软删引用节点 +
   * 源文件清理单次 IPC。供资产管理面板批量删除入口使用，替换逐资产串行
   * deleteFilmAsset 的 N+1 链路。
   */
  const batchDeleteFilmAssets = useCallback(
    async (assetIds: string[], options?: { hardDelete?: boolean }) => {
      if (assetIds.length === 0) return
      const result = await canvasApi.batchDeleteFilmAssets(projectId, assetIds, options)
      await applyCanvasMutationSnapshot(canvasApi.openSnapshot(projectId))
      return result
    },
    [applyCanvasMutationSnapshot, projectId],

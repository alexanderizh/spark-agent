...
const HIDE_SYSTEM_PROMPTS_KEY = 'spark-canvas:prompt-library:hide-system-prompts'
...
export function readHideSystemPrompts(storage?: Storage): boolean {
  const target = resolveStorage(storage)
  if (!target) return false
  try {
    return target.getItem(HIDE_SYSTEM_PROMPTS_KEY) === 'true'
  } catch {
    return false
  }
}

export function saveHideSystemPrompts(hidden: boolean, storage?: Storage): void {
  const target = resolveStorage(storage)
  if (!target) return
  try {
    target.setItem(HIDE_SYSTEM_PROMPTS_KEY, String(hidden))
  } catch {
    // Preferences are best-effort and must not block prompt library use.
  }
}

...
describe('pruneModelParamsForCanvas', () => {
  ...
  it('treats file attachments with media mimeType as reference-capable inputs', async () => {
    ;(canvasApi.listMediaModels as ReturnType<typeof vi.fn>).mockResolvedValueOnce({
      models: [
        {
          manifestId: 'xai:grok-imagine-video',
    ...
    await pruneModelParamsForCanvas({
      ...
      providerProfileId: 'profile-xai',
      modelId: 'grok-imagine-video',
      prompt: 'animate with reference footage',
      validateSubmission: true,
      modelParams: {},
      inputFiles: [
        { type: 'image', role: 'first_frame', url: 'https://example.com/first.png' },
        { type: 'file', mimeType: 'video/mp4', url: 'https://example.com/ref.mp4' },
      ],
    })

...
import { collectAssetReferences } from './assetLibrary/assetReferences'
...
export function CanvasAssetManagerPanel({
  ...
  // 引用反查：assetId → 引用它的节点列表（过滤 hidden 软删节点，缺陷 1）；assetId → 生成它的任务
  const referencesByAsset = useMemo(() => collectAssetReferences(nodes), [nodes])

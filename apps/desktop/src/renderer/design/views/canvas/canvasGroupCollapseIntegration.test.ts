...
const workspaceStyles = readFileSync(new URL('./CanvasWorkspaceView.less', import.meta.url), 'utf8')
...
describe('canvas collapsible group integration', () => {
  ...
  it('renders the folder cover and toggles group state without changing regular edit behavior', () => {
    ...
    expect(nodeSource).toContain('canvas-collapsed-group-back')
    expect(nodeSource).toContain('canvas-collapsed-group-insert')
    expect(nodeSource).toContain('canvas-collapsed-group-front')
    ...
    expect(nodeSource).toContain("node.title?.trim() || '编组'")
  })

  it('keeps the three-layer inserted-card motion accessible across themes', () => {
    expect(workspaceStyles).toContain("@import './CanvasCollapsedGroup.less';")

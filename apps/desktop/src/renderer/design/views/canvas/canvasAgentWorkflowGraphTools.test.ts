...
function baseSnapshot(): CanvasSnapshot {
  return {
    project: {
      id: 'project-1', userId: 0, title: '测试项目', status: 'active', settings: {},
      nodeCount: 0, assetCount: 0, taskCount: 0, createdAt: at, updatedAt: at,
    },
    board: {
      id: 'board-1', projectId: 'project-1', userId: 0, name: '主画布',
      viewport: { x: 0, y: 0, zoom: 1 }, settings: {}, createdAt: at, updatedAt: at,
    },
    activeBoardId: 'board-1',
    nodes: [], edges: [], assets: [], tasks: [],
  }
}
...
function graphInput() {
  return {
    ...
    nodes: [
      ...
      {
        ref: 'animate', role: 'operation', operation: 'image_to_video', title: '生成视频',
        prompt: '自然运镜', dependsOn: ['image-input'], isOutput: true,
...
function node(input: Partial<CanvasNode> & Pick<CanvasNode, 'id' | 'type'>): CanvasNode {
  return {
    projectId: 'project-1', boardId: 'board-1', userId: 0, title: input.id,
    x: 0, y: 0, width: 460, height: 300, rotation: 0, zIndex: 1,
    locked: false, hidden: false, data: {}, createdAt: at, updatedAt: at,
    ...input,
  }
}
...
function edge(sourceNodeId: string, targetNodeId: string): CanvasEdge {
  return {
    id: `edge-${sourceNodeId}-${targetNodeId}`, projectId: 'project-1', boardId: 'board-1',
    userId: 0, sourceNodeId, targetNodeId, type: 'used_as_input', taskId: null,
...
describe('canvas agent workflow graph tools', () => {
  it('registers atomic creation and read-only validation tools', () => {
    ...
    expect(schemas.canvas_create_reusable_workflow_graph?.paramsSchema).toHaveProperty(
      'required',
      ['name', 'nodes'],
    )
  ...
  it('plans and applies a connected graph in one workspace mutation', async () => {
    ...
    const applyTemplate = vi.fn(async (input: Parameters<CanvasWorkspaceActions['applyTemplate']>[0]) => {
      const image = node({
        id: 'created-image', type: 'image', title: '上传参考图',
        x: input.originX + input.nodes[0]!.x, y: input.originY + input.nodes[0]!.y,
        data: input.nodes[0]!.data ?? {},
      })
      const operation = node({
        id: 'created-operation', type: 'image_to_video', title: '生成视频',
        x: input.originX + input.nodes[1]!.x, y: input.originY + input.nodes[1]!.y,
        data: input.nodes[1]!.data ?? {}, taskId: 'task-1',
      })
      return {
        ...before,
    ...
    expect(applyTemplate.mock.calls[0]?.[0]).toMatchObject({
      ...
      nodes: [expect.objectContaining({ type: 'image' }), expect.objectContaining({ type: 'image_to_video' })],
  ...
  it('validates the current selection without requiring all canvas nodes to be connected', async () => {
    ...
    snapshot.nodes = [
      ...
      node({
        id: 'operation', type: 'image_to_video', taskId: 'task-1',
        x: 600, data: { operation: 'image_to_video', prompt: '自然运镜', subtype: 'workflow_output' },
      }),

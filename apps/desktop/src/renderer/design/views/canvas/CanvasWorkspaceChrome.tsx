...
import { StepModeSwitcher } from './stepStudio/StepModeSwitcher'

type CanvasWorkspaceChromeProps = {
  title: string
  /** 当前创作模式（画布 / 步骤），驱动顶栏模式切换器 */
  mode: CanvasProjectMode
  onSwitchMode: (mode: CanvasProjectMode) => void
  nodeCount: number
  assetCount: number
  taskCount: number
  ...
  return (
    ...
    >
      <div className="canvas-workspace-header-row">
        ...
        <span className="canvas-workspace-header-spacer" aria-hidden />
        <StepModeSwitcher mode={mode} onSwitch={onSwitchMode} />
      </div>
      <CanvasToolbar
        saveState={saveState}
        nodeCount={nodeCount}

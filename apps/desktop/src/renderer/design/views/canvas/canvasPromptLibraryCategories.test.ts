...
describe('canvas prompt library categories', () => {
  ...
  it('persists the hidden-system-prompts switch', () => {
    const values = new Map<string, string>()
    const storage = {
    ...
    expect(readHideSystemPrompts(storage)).toBe(false)
    saveHideSystemPrompts(true, storage)
    expect(readHideSystemPrompts(storage)).toBe(true)
    saveHideSystemPrompts(false, storage)
    expect(readHideSystemPrompts(storage)).toBe(false)
  })
})

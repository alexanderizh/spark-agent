...
export const CANVAS_AGENT_PRESETS: CanvasAgentPreset[] = [
  ...
  },
  {
    ...
    template: [
      ...
      '',
      '【完整性硬约束（最高优先级）】JSON 必须完整闭合后才算完成任务：镜头较多时宁可精简每个字段的文字量、压缩次要字段内容，也绝不允许 JSON 被截断。所有字符串值控制在必要长度内（description/shotPrompt ≤ 200 字，actionBeats 每拍 ≤ 40 字，其余字段 ≤ 80 字）。JSON 必须紧凑输出：不要缩进、不要多余空格换行（数组元素与对象字段依次连排）。',
      '',
      'JSON 顶层结构必须为：{"shots":[...],"summary":{"shotCount":数字,"totalDurationSec":数字}}。summary 必须是对象，不要写成字符串；shotCount 必须等于 shots 数组长度。',

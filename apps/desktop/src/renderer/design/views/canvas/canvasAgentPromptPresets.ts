...
export const CANVAS_AGENT_PRESETS: CanvasAgentPreset[] = [
  ...
  },
  {
    ...
    template: [
      ...
      '- characters：出场角色名数组；同时每个角色在 description 里刻画清楚',
      '- microExpression：人物微表情与表演细节（眼神方向、眉毛、嘴角、肌肉紧绷、呼吸、小动作、手持物品的姿态——细到可观测）',
      '- costume：角色服装/造型要点（颜色、材质、款式、配饰、与角色的呼应）',
      '- description：画面/动作的详细客观描述（现在时、可视、镜头内发生的一切：谁在做什么、如何做、画面构图、视觉重点、前后景关系）',
      '- dialogue：对白原文（无对白给空字符串）',
      '- narration：旁白/字幕（如有）',
      '- shotPrompt：面向 AI 图像/视频模型的完整英文或中文生成提示词（把以上所有视觉要素凝练成一段可直接用于生成的描述，包含主体、构图、镜头、光影、色彩、氛围、画质；务必自包含、可直接复制使用）',
      '字段无内容用空字符串或空数组，但 lighting/sceneLayout/blocking/microExpression/description/shotPrompt 这些核心视觉字段必须详尽，不得敷衍。',
      '',

  ...
  const onPointerMove = useCallback(
    (e: ThreeEvent<PointerEvent>) => {
      ...
      if (d.kind === 'ring' && d.axis !== undefined && d.ringPlaneNormal && d.ringCenter && d.startEuler) {
        ...
        // 软限位：默认钳到 JOINT_LIMITS，按住 Alt 可突破（与 mannequin.ts 的约定一致）。
        const clamped = clampJointEuler(d.jointId, next, { clamp: !e.nativeEvent.altKey })
        ...
        // 默认钳制软限位，按住 Alt 可突破（与 mannequin.ts 的约定一致）。
        const ikRes = solveIkToWorldTargetDirect(
          d.jointId,
          hit,
          metrics,
          jointRefs,

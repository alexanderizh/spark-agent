...
import { MIXAMO_BONE_BY_JOINT } from './MixamoActorRig'
import {
  deleteSavedPose,
  loadSavedPoses,
  renameSavedPose,
...
export function CanvasDirectorStage3DModal({
  ...
  const [draft, setDraftState] = useState<Stage3DData>(initial)
  // 脏检查用版本号而非 JSON.stringify：draft 含镜头截图/本地模型 base64 时，
  // 每次渲染全量序列化会卡顿（运镜播放/录制每帧都触发重渲染）。
  const draftVersionRef = useRef(0)
  const savedDraftVersionRef = useRef(0)
  const setDraft = useCallback(
    (value: Stage3DData | ((prev: Stage3DData) => Stage3DData)) => {
  ...
  // 弹窗卸载（关闭/切换节点）时取消进行中的录制：否则离屏 rAF 循环继续驱动
  // 已卸载的视口，录出全程黑帧并照样写回画布。
  useEffect(() => {
    return () => {
  ...
  useEffect(() => {
    ...
    setDraft(next)
    savedDraftVersionRef.current = draftVersionRef.current
  ...
  const isDirty = draftVersionRef.current !== savedDraftVersionRef.current
   ...
   * 基准按 rig 类型取：mixamo 素体渲染走 getMixamoPose（肩/臂等预设角度与 mannequin
   * 完全不同），用 mannequin 预设求 override 会让提交瞬间关节跳变。
   */
  const setActorJointEuler = useCallback((id: string, jointId: JointId, euler: Vec3) => {
    setDraft((d) => ({
      ...
      actors: d.actors.map((a) => {
        if (a.id !== id) return a
        const mixamoBone = a.rigType === 'mixamo' ? MIXAMO_BONE_BY_JOINT[jointId] : undefined
        const base = mixamoBone
          ? (getMixamoPose(a.pose).bones[mixamoBone] ?? [0, 0, 0])
          : (getPose(a.pose)[jointId] ?? [0, 0, 0])
        const joints = { ...(a.joints ?? {}) }
  ...
  const save = useCallback(async () => {
    ...
    try {
      ...
      message.error(error instanceof Error ? error.message : '保存 3D 场景失败')
      return
    }
    setDraft(next)
    savedDraftVersionRef.current = draftVersionRef.current

...
interface Props {
  ...
  /**
   * 画布节点 id → 任务状态（父级随画布快照刷新）。
   * 用于：① 时间线 clip 同步显示上游任务 running/failed 状态；
   * ② 节点被删除后把引用它的 canvas:/upstream: 资源标记为 missing（reconcile）。
   */
  taskStatusByNodeId?: ReadonlyMap<string, CanvasTaskStatus | undefined>
}
...
export function CanvasVideoWorkbenchModal({
  ...
  /** probe 失败过的源 URL：画布快照刷新会换 node 引用，避免同源反复重探。 */
  const probeFailedUrlRef = useRef('')

  // ── 画布节点删除/恢复 → 资源 missing 状态 reconcile ─────────────────
  // 资源面板是打开时的快照集合；画布上删除节点后，引用它的 canvas:/upstream: 资源
  // 原先会永久残留。这里按父级传入的节点索引把消失的节点对应资源标记为 missing
  // （节点恢复则清除），时间线 clip 已有 is-missing 红色样式。元数据写入不进 undo 栈。
  useEffect(() => {
    // 空索引 = 画布快照尚未加载，此时不能据此判定节点全部消失。
    if (!open || !taskStatusByNodeId || taskStatusByNodeId.size === 0) return
    updateProject((current) =>
      reconcileVideoWorkbenchResourceMissing(current, new Set(taskStatusByNodeId.keys())),
    )
  }, [open, taskStatusByNodeId, updateProject])

  // ── 资源 id → 上游任务状态（时间线 clip 着色用）─────────────────────
  const taskStatusByResourceId = useMemo(() => {
    if (!taskStatusByNodeId || taskStatusByNodeId.size === 0) return null
    const map = resolveVideoWorkbenchTaskStatusByResourceId(project, taskStatusByNodeId)
    return map.size > 0 ? map : null
  }, [project, taskStatusByNodeId])
    ...
    async (n: CanvasNode) => {
      ...
      try {
        ...
        if (res.success && res.result) {
          ...
          // 首次打开时主轨 clip 以 defaultImageDurationSec(8s) 占位。probe 拿到真实时长后
          // 必须先扩展占位 clip、再回填 resource.durationSec —— 顺序反了会让
          // backfillVideoWorkbenchProjectResourceMetadata 的 isUntouchedFallback 失效
          //（resource 已有 duration 后不再扩展），占位 clip 就永远停在 8 秒。
          // 画布视频节点资源多带 thumbnailUrl，缩略图走 <img> 分支不回传 metadata，
          // probe 是占位 clip 拿到真实时长的唯一可靠入口。
          const sourceResourceId = `source:${n.id}`
          updateProject((current) =>
          ...
          updateDraft((d) => ({
            ...d,
            probeInfo,
            resourcePanel: backfillResourceMetadata(d.resourcePanel, sourceResourceId, probeInfo),
          }))
        } else {
          // probe 返回失败（路径校验/ffmpeg 执行错误）—— 不阻塞，用 video 元素信息降级
          console.warn('[video-workbench] probe failed:', res.error)
          probeFailedUrlRef.current = sourcePath
          setProbeFailed(true)
        }
      } catch (err) {
  ...
  useEffect(() => {
    ...
    if (
      ...
      ffmpegReady !== true ||
      probingRef.current ||
      // 画布快照每次刷新都会换 node 引用；同源 probe 失败后不再重试，
      // 避免反复拉起 ffmpeg 子进程。
      probeFailedUrlRef.current === sourceUrl
    )
      return
  ...
  const extractKeyframes = useCallback(
    async (strategy: KeyframeStrategy) => {
      if (!node || !probe) {
        // 按钮只按 ffmpeg 就绪禁用；probe 缺失时静默 return 会让用户觉得"点了没反应"。
        message.warning(
          probeFailed
            ? '视频信息探测失败，无法提取关键帧；请先确认源视频文件可访问后重开工作台'
            : '视频信息尚未就绪，请稍候再试',
        )
        return
      }
    ...
    [node, probe, probeFailed, extractConfig, updateDraft],
  )
  ...
  const handleRemoveClip = useCallback(
    (clipId: string) => {
      ...
      // 连播中删除当前片段会让播放状态机失去 currentClip 而停摆，先退出连播。
      if (playback.currentClipId === clipId) playbackExit()
      const result = applyProjectCommand({ type: 'clip/remove', clipId })
      if (!result.applied) message.warning('轨道已锁定，无法删除片段')
    },
    [applyProjectCommand, playback.currentClipId, playbackExit],
  )
  ...
  const handleRemoveResource = useCallback(
    (resourceId: string) => {
      ...
      // 连播引用该资源时先退出连播，避免播放头冻结在已删除的片段上。
      if (playback.active) playbackExit()
      updateProject(
        (current) => ({
          ...current,
    ...
    [playback.active, playbackExit, updateProject],
  )
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      if (e.key === 'Escape') {
        e.preventDefault()
        e.stopPropagation()
        // 先关最上层的资源选择器（antd Modal 自己的 Esc 处理已被本 window capture
        // 监听器拦下），否则按一次 Esc 会把整个工作台关掉。
        if (canvasPickerOpen) {
          setCanvasPickerOpen(false)
          return
        }
        ...
        <div className="vwb-body">
          ...
          <div className="vwb-preview-pane">
              ...
              {previewUrl && previewKind === 'video' ? (
                <video
                  ...
                  onLoadedMetadata={(e) => {
                    ...
                    // probe 失败/ffmpeg 缺失时的兜底：主源预览的 <video> 元素同样拿得到
                    // 真实时长，复用资源面板的 metadata 回填管线（buffer + flush +
                    // backfillVideoWorkbenchProjectResourceMetadata），把默认 8s 占位
                    // clip 扩展到真实时长；probe 成功路径已扩展过时这里是 no-op。
                    if (node && !selectedResource && !isPlayback && Number.isFinite(d) && d > 0) {
                      handleResourceMeta(`source:${node.id}`, {
                        durationSec: d,
            ...
            <VideoWorkbenchMultiTrackTimeline
              ...
              taskStatusByResourceId={taskStatusByResourceId}
              canUndo={canUndo}
              canRedo={canRedo}

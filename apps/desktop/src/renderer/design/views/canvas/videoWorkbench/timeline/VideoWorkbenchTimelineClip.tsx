...
interface Props {
  ...
  /** 上游画布任务状态（running/failed）；undefined 表示无任务或状态不可用。 */
  taskStatus?: 'running' | 'failed' | undefined
...
function VideoWorkbenchTimelineClipComponent({
  ...
  return (
    <div
      className={`vwb-mt-clip vwb-mt-clip-${track.kind}${
        resource ? ` vwb-mt-clip-res-${resource.kind}` : ''
      }${selected ? ' is-selected' : ''}${
        track.locked || editingDisabled ? ' is-locked' : ''
      }${resource?.missing || !resource ? ' is-missing' : ''}${
        taskStatus === 'running' ? ' is-task-running' : ''
      }${taskStatus === 'failed' ? ' is-task-failed' : ''}`}
      style={{
      ...
      title={`${resource?.title ?? '资源已丢失'} · ${formatTimestamp(clip.durationSec)}${
        taskStatus === 'running' ? ' · 上游任务生成中' : ''
      }${taskStatus === 'failed' ? ' · 上游任务失败' : ''}`}
    >
      <button
        type="button"

...
export type CanvasFlowNodeData = {
  ...
  actions: {
    ...
    /** 图片节点/图片任务：右键/工具栏 → 尺寸与压缩弹窗（sharp 处理，物化新子节点） */
    scaleCompressImage?: (nodeId: string) => void
...
export const CanvasNode = memo(function CanvasNode({
  ...
  const canScaleCompressImage =
    isImageContent &&
    hasOperationOutput &&
    Boolean(contentNode?.data.url || contentNode?.data.filePath) &&
    Boolean(actions.scaleCompressImage)
  ...
  const selectionToolbarEntries = useMemo<CanvasNodeToolbarEntry[]>(() => {
    ...
    if (canScaleCompressImage) {
      mediaChildren.push({
        key: 'scale-compress-image',
        label: '尺寸与压缩',
        icon: <Icons.Minimize size={15} />,
        onClick: () => actions.scaleCompressImage?.(node.id),
      })
    }

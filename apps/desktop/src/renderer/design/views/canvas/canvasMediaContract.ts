...
function prefersReferenceVideoCapability(
  ...
  if (operation === 'text_to_video') {
    return files.some((file) => isMediaFile(file))
  }
  // 与主进程 media-router.service.ts prefersReferenceCapability 保持同口径：
  // type:'file' + 媒体 mimeType 的附件同样按媒体输入参与判定，避免两端
  // 预检能力与执行能力不一致。
  if (files.some((file) => isMediaFile(file) && file.type !== 'image')) return true
  const images = files.filter((file) => isMediaFile(file) && file.type === 'image')

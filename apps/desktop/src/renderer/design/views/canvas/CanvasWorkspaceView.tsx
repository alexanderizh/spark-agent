...
import { ProjectAssetLibrary } from './assetLibrary/ProjectAssetLibrary'
...
import { CanvasImageScaleCompressModal } from './imageNode/CanvasImageScaleCompressModal'
import { useCanvasImageScaleCompress } from './imageNode/useCanvasImageScaleCompress'
...
import { StepStudioView } from './stepStudio/StepStudioView'
import {
  readStepStudioMeta,
  readStepStudioState,
  stepStudioMetaPatch,
  stepStudioStatePatch,
} from './stepStudio/stepStudioMeta'
import type { CanvasMediaTaskSubmitter } from './assetLibrary/assetCreateTypes'
import { applyAssemblyToProject, type StepAssemblySource } from './stepStudio/stepAssemblyModel'
import type {
...
/** 视频工作台节点索引的稳定空值（快照未加载时避免传新 Map 触发多余 reconcile）。 */
const EMPTY_TASK_STATUS_MAP: ReadonlyMap<string, CanvasTaskStatus | undefined> = new Map()
...
export function CanvasWorkspaceView({
  ...
  // P2 新资产库（画布 / 步骤模式共用入口），旧资产中心保留为重编辑过渡入口
  ...
  const closeCanvasFloatPanels = useCallback(
    (
      ...
      if (except !== 'film-center' && except !== 'asset-library') setFilmCenterOpen(false)
      if (except !== 'asset-library') setAssetLibraryOpen(false)
  ...
  const imageScaleCompress = useCanvasImageScaleCompress({
    projectId,
    snapshotRef,
    viewportControlsRef: canvasViewportControlsRef,
    setSelectedNodeIds,
  ...
  // 「展开产物」的核心物化管线：布局计划（幂等）→ insertAsset 落节点 →
  // 写入 materializedOutput 溯源 → references 连线。不含选中/聚焦/提示等 UI 副作用，
  // 供「展开产物」与「视频编辑」等入口共用；已物化产物通过计划幂等复用现有节点。
  const materializeOperationOutputs = useCallback(
    async (operationNodeId: string, outputs: CanvasOperationOutputView[]) => {
      const current = snapshotRef.current
      if (!current || outputs.length === 0) return null
      ...
      return {
        materializedNodeIds,
        createdNodeIds,
        unsupportedOutputIds: plan.unsupportedOutputIds,
      }
    },
  ...
  const handleEditVideo = useCallback(
    async (nodeId: string) => {
      ...
      // 目标是视频节点或有视频 url 才能打开工作台
      const isVideoTarget =
        target.type === 'video' ||
        target.data.subtype === 'video_workbench' ||
        (typeof target.data.url === 'string' && Boolean(target.data.url))
      if (!isVideoTarget) {
        message.warning('该节点没有可编辑的视频内容')
        return
      }

      // 操作节点的产物可能只有 asset、尚未物化成画布节点：resolve 返回的是
      // `operation-output:` 虚拟视图节点，该 id 不在 db.nodes 中，直接
      // setVideoWorkbenchNodeId 会因 Modal 查不到节点而静默失败。
      // 此时复用「展开产物」管线先把主产物物化成真实节点（幂等），再打开工作台。
      if (!snap.nodes.some((item) => item.id === target.id)) {
        const runs = buildCanvasOperationRunViews(node, snap)
        const state = resolveCanvasOperationOutputState(node, runs)
        const result = state.primaryOutput
          ? await materializeOperationOutputs(node.id, [state.primaryOutput])
          : null
        const materializedNodeId = result?.materializedNodeIds[0]
        if (!materializedNodeId) {
          message.warning('该节点没有可编辑的视频内容')
          return
        }
        if (result.createdNodeIds.length > 0) {
          message.success('已将产物展开为视频节点')
        }
        closeCanvasFloatPanels('node-edit')
        setSelectedNodeIds([materializedNodeId])
        setVideoWorkbenchNodeId(materializedNodeId)
        return
      }
    ...
    [closeCanvasFloatPanels, materializeOperationOutputs],
  )
  ...
  const handleAudioTrim = useCallback(
    async (nodeId: string, startSec: number, endSec: number) => {
      ...
      try {
        ...
        if (created) {
          await refreshTaskSnapshot()
          // 与压缩/其他物化链路一致：选中并聚焦新节点，确保用户立刻看到产物。
  ...
  const handleVideoScaleCompressConfirm = useCallback(
    async (input: VideoScaleCompressConfirmInput) => {
      ...
      await refreshTaskSnapshot()
      // 期望落点（父节点右侧）被占用时，碰撞规避会把副本挪到远处空位（可达数千像素）。
      // 选中并聚焦新节点，确保用户始终能立刻看到压缩产物（与其他物化链路一致）。
      setSelectedNodeIds([created.id])
      requestAnimationFrame(() => canvasViewportControlsRef.current?.focusNodes([created.id]))
      message.success(
  ...
  const handleExpandOperationOutputs = useCallback(
    async (operationNodeId: string, outputs: CanvasOperationOutputView[]) => {
      const result = await materializeOperationOutputs(operationNodeId, outputs)
      if (!result) return
      if (result.unsupportedOutputIds.length > 0) {
        message.warning(`${result.unsupportedOutputIds.length} 个产物尚未关联资产，暂不能展开`)
      }
      if (result.materializedNodeIds.length > 0) {
        setSelectedNodeIds(result.materializedNodeIds)
        requestAnimationFrame(() => {
          canvasViewportControlsRef.current?.focusNodes(result.materializedNodeIds, {
        ...
        message.success(
          result.createdNodeIds.length > 0
            ? `已展开 ${result.createdNodeIds.length} 个资产引用节点`
            : '这些产物已经在画布中展开',
        )
      }
    },
    [materializeOperationOutputs],
  )
  ...
  // P6：步骤模式「组装时间线」。幂等复用 stepStudioState.assemblyNodeId 指向的
  // video_workbench 节点（被删则重建）；组装结果只替换 step 标记的资源/片段，
  // 用户在工作台手动添加的内容原样保留（见 stepAssemblyModel）。
  const assembleStepTimeline = useCallback(
    async (sources: StepAssemblySource[]): Promise<boolean> => {
      if (!snapshot) return false
      const projectId = snapshot.project.id
      const currentState = readStepStudioState(snapshot.project)
      const existing =
        currentState?.assemblyNodeId != null
          ? (snapshot.nodes.find((item) => item.id === currentState.assemblyNodeId) ?? null)
          : null

      let nodeId: string
      let newNodeData: Record<string, unknown> | null = null
      if (existing) {
        nodeId = existing.id
      } else {
      ...
      // 存量节点基于其当前工程增量组装；新节点从空数据起步（解析失败自动落默认工程）
      const rawWorkbench = existing
        ? (existing.data as { videoWorkbench?: unknown }).videoWorkbench
        : undefined
      const assembled = applyAssemblyToProject(rawWorkbench, sources)
      await updateNodeData(nodeId, {
        // 新建节点需补 subtype/displayCategory 与基础 data；存量节点 merge 语义下只传变更字段
        ...(newNodeData ?? {}),
        ...(newNodeData ? { subtype: 'video_workbench', displayCategory: 'content' } : {}),
        text: '步骤模式成片：双击打开视频工作台精剪。',
        videoWorkbench: assembled as unknown as Record<string, unknown>,
      })

      // 回填 assemblyNodeId（首次组装 / 节点重建后漂移修正），sequences 原样保留
      if (currentState?.assemblyNodeId !== nodeId) {
        const nextState: StepStudioState = {
          schemaVersion: 1,
          sequences: currentState?.sequences ?? [],
          assemblyNodeId: nodeId,
        }
        await canvasApi.updateProjectMetadata(projectId, stepStudioStatePatch(nextState))
      }
      await refreshCanvasSnapshot()
      return true
    },
    [snapshot, createTextNode, patchNodes, updateNodeData, refreshCanvasSnapshot],
  )
  ...
  // P3：步骤模式（Step Studio）模式分流回调。
  // 元数据存 project.metadata.stepStudio（缺省容错 → 画布模式）；模式切换写回后
  // saveCanvas 落库（项目级记忆，重开项目恢复上次模式），步骤内切换不落盘。
  const switchProjectMode = useCallback(
    async (mode: CanvasProjectMode) => {
      if (!snapshot) return
      const current = readStepStudioMeta(snapshot.project)
      if (current.lastMode === mode) return
      await updateProjectMetadata(stepStudioMetaPatch({ ...current, lastMode: mode }))
      await saveCanvas()
    },
    [snapshot, updateProjectMetadata, saveCanvas],
  )

  const selectStepStudioStep = useCallback(
    async (step: StepStudioStageKey) => {
      if (!snapshot) return
      const current = readStepStudioMeta(snapshot.project)
      if (current.lastMode === 'step' && current.activeStep === step) return
      await updateProjectMetadata(stepStudioMetaPatch({ lastMode: 'step', activeStep: step }))
    },
    [snapshot, updateProjectMetadata],
  )

  // 设定步骤的生成任务提交（设定图生成走 filmOutput 回填资产）
  const handleStepSetupCreateTask = useCallback(
    async (
      request: Parameters<CanvasMediaTaskSubmitter>[0],
      options: Parameters<CanvasMediaTaskSubmitter>[1],
    ) => {
      if (!snapshot) return
      await canvasApi.createMediaTask(snapshot.project.id, request, {
        ...(options?.filmOutput ? { filmOutput: options.filmOutput } : {}),
      })
      await refreshCanvasSnapshot()
    },
    [snapshot, refreshCanvasSnapshot],
  )

  // 分镜步骤的提交 / 状态写回。必须固化（useCallback）而非在渲染处内联：
  // StepStoryboardView 的草稿防抖（700ms）与卸载冲刷依赖这些 props 的引用
  // 稳定性，内联箭头函数每次渲染换新引用会击穿防抖、把草稿逐次立即落盘。
  const handleStepStoryboardCreateTask = useCallback(
    async (
      request: Parameters<CanvasMediaTaskSubmitter>[0],
      options: Parameters<CanvasMediaTaskSubmitter>[1],
    ) => {
      if (!snapshot) return
      await canvasApi.createMediaTask(snapshot.project.id, request, {
        ...(options?.createdTaskRef ? { createdTaskRef: options.createdTaskRef } : {}),
      })
      await refreshCanvasSnapshot()
    },
    [snapshot, refreshCanvasSnapshot],
  ...
  const handleStepStoryboardUpdateState = useCallback(
    async (state: StepStudioState) => {
      if (!snapshot) return
      await canvasApi.updateProjectMetadata(snapshot.project.id, stepStudioStatePatch(state))
      await refreshCanvasSnapshot()
    },
    [snapshot, refreshCanvasSnapshot],
  )

  // 打开步骤工程挂载的工作台节点；节点可能已被用户在画布侧删除，打开前校验
  const handleOpenStepWorkbench = useCallback(() => {
    if (!snapshot) return
    const target = readStepStudioState(snapshot.project)?.assemblyNodeId
    if (target == null) {
      message.warning('请先组装时间线，再进入工作台精剪')
      return
    }
    const nodeExists = snapshot.nodes.some((node) => node.id === target)
    if (!nodeExists) {
      message.warning('工作台节点已被删除，请重新组装时间线后再进入')
      return
    }
    setVideoWorkbenchNodeId(target)
  }, [snapshot])
  ...
  // 画布节点 id → 任务状态：视频工作台用它同步时间线片段的生成中/失败状态，
  // 并在节点被删除后把引用资源标记为 missing（节点存在性以本表为准）。
  // 注意：必须收录全部节点 id（值可为 undefined），只收录带 status 的节点会把
  // 普通素材节点的资源误判为已删除。
  const workbenchTaskStatusByNodeId = useMemo(() => {
    const nodes = snapshot?.nodes ?? []
    if (nodes.length === 0) return EMPTY_TASK_STATUS_MAP
    const map = new Map<string, CanvasTaskStatus | undefined>()
  ...
  // P3：步骤模式（Step Studio）模式分流。
  const stepStudioMeta = readStepStudioMeta(snapshot.project)

  if (stepStudioMeta.lastMode === 'step') {
    return (
      <CanvasOverlayBoundary className="canvas-workspace canvas-cinematic">
        <StepStudioView
          projectTitle={snapshot.project.title}
          activeStep={stepStudioMeta.activeStep}
          snapshot={snapshot}
          setupActions={{
            onCreateFilmAsset: createFilmAsset,
            onUploadImageAsset: uploadImageAsset,
            onCreateMediaTask: handleStepSetupCreateTask,
            refreshSnapshot: refreshCanvasSnapshot,
          }}
          storyboardActions={{
            onCreateMediaTask: handleStepStoryboardCreateTask,
            onUpdateState: handleStepStoryboardUpdateState,
            refreshSnapshot: refreshCanvasSnapshot,
          }}
          assemblyActions={{
            onAssemble: assembleStepTimeline,
            onOpenWorkbench: handleOpenStepWorkbench,
          }}
          onSelectStep={(step) => void selectStepStudioStep(step)}
          onSwitchToCanvas={() => void switchProjectMode('canvas')}
        ...
        />
        {/* 工作台精剪 Modal 与画布模式共用同一实例（key 随节点切换重建） */}
        <CanvasVideoWorkbenchModal
          ...
          taskStatusByNodeId={workbenchTaskStatusByNodeId}
        />
      </CanvasOverlayBoundary>
    )
  }

  // P2：旧资产中心与新项目资产库（ProjectAssetLibrary）共用的操作回调。
  // 原本内联在 CanvasFilmAssetCenter 的 handlers 属性上，抽出为共享 const
  // （每次渲染重建，与原内联对象语义一致）。
  const filmCenterHandlers: FilmCenterHandlers = {
    createFilmAsset,
    updateFilmAsset,
    deleteFilmAsset,
    getFilmAssetUsage,
    ...
    onLocateAsset: (assetId) => {
      ...
      // 资产中心 / 资产库覆盖在画布上方，先关闭后再聚焦，确保用户能直接看到目标节点。
      setFilmCenterOpen(false)
  ...
  return (
    ...
    >
      <CanvasWorkspaceChrome
        ...
        mode={stepStudioMeta.lastMode}
        onSwitchMode={(mode) => void switchProjectMode(mode)}
      ...
      <div className="canvas-workspace-body" style={sidePanelStyle}>
        ...
        >
          ...
          <CanvasStage
            ...
            onImageScaleCompress={imageScaleCompress.open}
          ...
          <CanvasBottomDock
            ...
            onOpenFilmCenter={() => {
              // P2：Dock 入口切到新项目资产库；旧中心经库内「完整文稿 / 剧本编辑」过渡进入
              closeCanvasFloatPanels('asset-library')
          ...
          />
          <CanvasImageScaleCompressModal
            key={imageScaleCompress.source?.nodeId ?? 'closed'}
            open={Boolean(imageScaleCompress.source)}
            source={imageScaleCompress.source}
            onClose={imageScaleCompress.close}
            onConfirm={imageScaleCompress.confirm}
          />
          ...
          <ProjectAssetLibrary
            ...
            onDataMutated={() => void refreshCanvasSnapshot()}
          />
        ...
        <CanvasWorkspaceSidePanel
          ...
          onDeleteAssets={async (assetIds) => {
            // 批删走单次遍历 + 单次清理 IPC（assetLibrary 缺陷 4）；引用节点由
            // batchDelete 内部级联软删并清理连线/任务引用。
            await batchDeleteFilmAssets(assetIds, { hardDelete: true })
          }}

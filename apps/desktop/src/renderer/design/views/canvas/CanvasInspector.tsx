...
import { isEduUploadAssetUrl } from '@spark/shared'
import type {
  CanvasAsset,
  CanvasEdge,
  CanvasNode,
...
function AssetInspector({ asset }: { asset: CanvasAsset }) {
  // 仅云端中转 URL（Spark 平台 uploads）提示：该类文件为临时存储，服务端会定期清理。
  const isTemporaryCloudUrl = isEduUploadAssetUrl(asset.url)
  return (
    <div className="canvas-task-param-panel">
      ...
      {isTemporaryCloudUrl && (
        <div className="canvas-operation-panel-hint">
          该资源使用平台临时云存储（用于渠道中转），服务端将定期清理过期文件；如需长期保留，请下载到本地或保存为本地素材。
        </div>
      )}
    </div>
  )

...
const mocks = vi.hoisted(() => {
  ...
  const deleteFilmAsset = vi.fn(async (_projectId: string, assetId: string) => {
...
vi.mock('./canvas.api', () => ({
  canvasApi: {
    ...
    deleteFilmAsset: mocks.deleteFilmAsset,
  },
}))
...
beforeEach(() => {
  ...
  mocks.deleteFilmAsset.mockReset()
  // 跨测试时，openSnapshot 闭包内的 deletedIds 会累积，污染后续测试的
  // 初始资源列表；mockClear 不动闭包变量，要手动清空。
  mocks.deletedIds.clear()
  // 重置 deleteFilmAsset 的默认实现：mockReset 会清掉 vi.hoisted 里设置的
  // async impl，不补回去的话后续 await deleteFilmAsset(...) 不会真推进
  // microtask，导致 setAssetRev 之后的 useEffect 重拉永远不结束。
  mocks.deleteFilmAsset.mockImplementation(async (_projectId: string, assetId: string) => {
    mocks.deletedIds.add(assetId)
  })
...
describe('CanvasProjectDetail multi-select and batch delete', () => {
  ...
  it('opens a confirm dialog with the right title and calls deleteFilmAsset per id', async () => {
    ...
    // 两个资源都走 deleteFilmAsset
    expect(mocks.deleteFilmAsset).toHaveBeenCalledTimes(2)
    expect(mocks.deleteFilmAsset).toHaveBeenNthCalledWith(1, 'project-1', 'a-1', {
      hardDelete: true,
    })
    expect(mocks.deleteFilmAsset).toHaveBeenNthCalledWith(2, 'project-1', 'a-3', {
      hardDelete: true,
    })
  ...
  it('keeps failed assets in the selection so users can retry', async () => {
    mocks.deleteFilmAsset
      .mockResolvedValueOnce(undefined) // a-1 成功
      .mockRejectedValueOnce(new Error('boom')) // a-2 失败
      .mockResolvedValueOnce(undefined) // a-3 成功
    await renderDetail()
    await waitForAssets()
    ...
    expect(mocks.deleteFilmAsset).toHaveBeenCalledTimes(3)
    // 单条失败触发 error，3 个里 2 个成功 → success 报 2
    expect(mocks.messageSuccess).toHaveBeenCalledWith('已删除 2 个资源')
    expect(mocks.messageWarning).toHaveBeenCalled()

    // a-2 仍带 is-selected，a-1 / a-3 已取消
    expect(findTile('a-1')?.className).not.toContain('is-selected')
    expect(findTile('a-2')?.className).toContain('is-selected')
    expect(findTile('a-3')?.className).not.toContain('is-selected')
  })

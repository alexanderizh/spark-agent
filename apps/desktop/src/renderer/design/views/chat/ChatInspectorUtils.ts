...
export function extractInspectorTasks(
  ...
  // SDK 的任务 id 只在单个轮次内唯一，每个新轮次都从 #1 重新计数；host 与
  // 各成员的清单也彼此独立。因此按任务流（host 或成员）分桶维护：桶内
  // TaskCreate 命中已存在 id 即代表新一轮清单开始，需整体替换旧快照，否则
  // 后续轮次的任务会被上一轮同号任务去重吞掉，界面停留在旧内容。
  const tasksByStream = new Map<string, Map<string, InspectorTask>>()
  let nextSeq = 0
  let fallbackCounter = 0

  const streamKeyOf = (block: ToolCallBlock): string =>
    block.teamMemberContext != null ? `member:${block.teamMemberContext.memberAgentId}` : 'host'
  ...
  for (const message of messages) {
    for (const block of message.blocks) {
      ...
      const streamKey = streamKeyOf(block)
      let tasks = tasksByStream.get(streamKey)
      if (tasks == null) {
        tasks = new Map<string, InspectorTask>()
        tasksByStream.set(streamKey, tasks)
      }
      ...
      if (lower === 'task_create' || lower === 'taskcreate') {
        ...
        // 轮内 id 单调递增不会自我碰撞；命中已存在 id 只会来自新一轮的
        // 重新编号，旧清单已被新一轮完全取代，清空后重新累积。
        if (tasks.has(id)) tasks.clear()
        tasks.set(id, {
          id,
          subject,
  ...
  return Array.from(tasksByStream.values())
    .flatMap((tasks) => Array.from(tasks.values()))

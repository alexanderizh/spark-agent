 ...
 *
 * claude 例外：usage_update 的 input + cache（见 providerContextWindowTokens）就是
 * 最近一次请求的真实 prompt 规模，比本地估算准——估算只覆盖本轮用户消息与
 * system prompt，完全不含图片、历史消息和工具输出（多图会话曾因此把 ~97% 的
 * 真实占用显示成 2%）。因此 claude 有真实值时真实值优先，估算只在尚无真实
 * 值（新轮首个请求未返回）时兜底。
 */

...
describe('ChatViewUtils', () => {
  ...
  it('prefers real claude usage over local estimates that ignore images and history', () => {
    expect(
      resolveContextUsedTokens({
        provider: 'claude',
        ledgerEstimatedTokens: 12_000,
        turnEstimatedTokens: 8_000,
        providerInputTokens: 193_305,
      }),
    ).toBe(193_305)
    // 尚无真实值（新轮首个请求未返回）时仍用估算兜底
    expect(
      resolveContextUsedTokens({
        provider: 'claude',
        turnEstimatedTokens: 8_000,
    ...
    // codex 的 inputTokens 是轮内累计消耗，维持估算优先
    expect(
      resolveContextUsedTokens({
        provider: 'codex',
        ledgerEstimatedTokens: 12_000,

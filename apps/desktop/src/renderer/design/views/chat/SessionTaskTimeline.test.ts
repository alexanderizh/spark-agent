...
describe('session task timeline', () => {
  ...
  it('shows the newest turn snapshot when later turns restart task numbering from #1', () => {
    const firstTurn = [
      tool(
        'turn1-create-a',
        'task_create',
        { subject: '旧轮任务一' },
        'Task #1 created successfully: 旧轮任务一',
      ),
      tool(
        'turn1-create-b',
        'task_create',
        { subject: '旧轮任务二' },
        'Task #2 created successfully: 旧轮任务二',
      ),
      tool('turn1-complete', 'task_update', { taskId: '1', status: 'completed' }),
    ]
    const secondTurn = [
      tool(
        'turn2-create-a',
        'task_create',
        { subject: '新轮任务一' },
        'Task #1 created successfully: 新轮任务一',
      ),
      tool(
        'turn2-create-b',
        'task_create',
        { subject: '新轮任务二' },
        'Task #2 created successfully: 新轮任务二',
      ),
    ]
    const messages = [
      assistant('assistant-1', 'completed', firstTurn),
      assistant('assistant-2', 'streaming', secondTurn),
    ]

    const timeline = buildSessionTaskTimeline(messages)

    expect(timeline.get('assistant-1')?.tasks.map((task) => task.subject)).toEqual([
      '旧轮任务一',
      '旧轮任务二',
    ])
    expect(timeline.get('assistant-2')?.tasks.map((task) => task.subject)).toEqual([
      '新轮任务一',
      '新轮任务二',
    ])
  })
  ...
  it('compares rebuilt task snapshots by value for memoized history rows', () => {
    const entry = {
      ...
      tasks: [{ id: '1', subject: '加载历史任务', status: 'completed' as const, createdAt: 0 }],
    }
...
describe('shouldReplaceSessionTaskBlock', () => {
  const entry = {
    ...
    tasks: [{ id: '1', subject: '验证任务协议', status: 'in_progress' as const, createdAt: 0 }],
  }
  ...
  it('never routes team member task blocks or non-tool blocks', () => {
    ...
    expect(
      shouldReplaceSessionTaskBlock({ kind: 'text', content: '正文', isStreaming: false }, entry),
    ).toBe(false)
  })

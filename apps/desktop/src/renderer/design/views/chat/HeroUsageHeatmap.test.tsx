...
describe('resolveEmptyHeroUsageMode', () => {
  it('maps loading / error / active days onto hero layout modes', () => {
    ...
    expect(resolveEmptyHeroUsageMode(false, null, 4)).toBe('cards')
...
describe('useEmptyHeroUsage', () => {
  ...
  it('keeps quick cards when active days are at or below the threshold', async () => {
    const invoke = vi.fn().mockResolvedValue({ dailyGroups: makeSparseActiveDays(4) })
    ;(window as unknown as { spark: { invoke: typeof invoke } }).spark = { invoke }

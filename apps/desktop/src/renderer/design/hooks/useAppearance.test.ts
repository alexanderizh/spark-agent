...
describe('patchAppearance ui zoom', () => {
  ...
  it('persists the absolute browser zoom reported by the main process', () => {
    ...
    act(() => {
      window.dispatchEvent(
        new CustomEvent('spark:browser-zoom-changed', { detail: { zoomPercent: 105 } }),
      )
    })
  ...
  it('applies synchronized appearance without writing persisted settings again', () => {
    applySyncedAppearanceLocally({ fontSize: 17, uiZoom: 115 })

    expect(document.documentElement.style.getPropertyValue('--font-base')).toBe('17px')
    expect(invoke).toHaveBeenCalledWith('window:set-zoom', { zoomPercent: 115 })

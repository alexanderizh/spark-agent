...
/**
 * Apply a server-synchronized appearance to renderer state only.
 * The main process has already persisted the canonical value in SQLite, so this
 * path must not issue another settings:set that could race and restore stale fields.
 */
export function applySyncedAppearanceLocally(patch: Partial<AppearanceSettings>): void {
  const next = { ...readAppearance(), ...patch }
  if (patch.uiZoom != null) next.uiZoom = clampUiZoom(patch.uiZoom)
...
function applyAppearance(settings: AppearanceSettings) {
  ...
  root.style.setProperty(
    '--code-ligature',
    settings.codeLigature ? '"calt", "liga", "dlig", "ss02", "zero"' : 'normal',
  )
  ...
  root.style.setProperty(
    '--backdrop-blur',
    settings.backdropBlur ? 'blur(24px) saturate(1.4)' : 'none',
  )
...
function clampUiZoom(value: number): number {
  return Math.min(
    UI_ZOOM_MAX,
    Math.max(UI_ZOOM_MIN, Math.round(value / UI_ZOOM_STEP) * UI_ZOOM_STEP),
  )
}

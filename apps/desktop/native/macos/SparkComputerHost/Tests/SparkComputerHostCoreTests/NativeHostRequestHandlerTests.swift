...
final class NativeHostRequestHandlerTests: XCTestCase {
  ...
  func testReturnsObservationWithAdjacentPngAndExecutesBoundAction() async throws {
    ...
    XCTAssertEqual(resultObject["executionChannel"] as? String, "background_ax")
  }

  func testActionResultOmitsExecutionChannelWhenUnreported() async throws {
    let provider = FakePlatformProvider(
      controlAvailable: true, executionChannel: nil)
    let handler = NativeHostRequestHandler(provider: provider)
    let result = try await handler.handle(try executeActionRequest())
    let resultObject = try responseObject(result.json)
    XCTAssertEqual(resultObject["status"] as? String, "executed")
    XCTAssertFalse(
      resultObject.keys.contains("executionChannel"),
      "action_result must stay byte-compatible when the host does not report a channel")

    let foregroundProvider = FakePlatformProvider(
      controlAvailable: true, executionChannel: .foregroundCG)
    let foregroundResult = try await NativeHostRequestHandler(provider: foregroundProvider)
      .handle(try executeActionRequest())
    XCTAssertEqual(
      (try responseObject(foregroundResult.json))["executionChannel"] as? String,
      "foreground_cg"
    )
  }
...
private actor FakePlatformProvider: NativeHostPlatformProviding {
  ...
  private let executionChannel: NativeExecutionChannel?
  ...
  init(
    ...
    executionChannel: NativeExecutionChannel? = .backgroundAX
  ) {
  ...
  func executeAction(_ envelope: NativeComputerActionEnvelope) async throws -> NativeActionExecution {
    ...
    executed += 1
    return NativeActionExecution(status: .executed, executionChannel: executionChannel)
  }

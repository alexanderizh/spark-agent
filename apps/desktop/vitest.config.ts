...
// monaco-editor 的 package.json 没有 main/exports 字段（仅 module），生产构建走
// rollup 可解析，但 vitest 的 vite 包入口解析会抛 "Failed to resolve entry"。
// 即便指到真实 ESM 入口，其模块图在 vitest 下加载也远超冒烟测试 5s 超时。
// 单元测试不验证 monaco 本身，alias 到最小 stub（含 worker 副路径）。
const monacoEntryPath = resolve(
  dirname(nodeRequire.resolve('monaco-editor/package.json')),
  'esm/vs/editor/editor.main.js',
)
const monacoStubPath = resolve(__dirname, 'src/renderer/tests/__mocks__/monaco-editor-stub.ts')
const monacoWorkerStubPath = resolve(
  __dirname,
  'src/renderer/tests/__mocks__/monaco-worker-stub.ts',
)
...
export default defineConfig({
  test: {
    globals: true,
    environment: 'node',
    setupFiles: ['src/renderer/tests/setup-jsdom-globals.ts'],
  ...
  resolve: {
    alias: [
      ...
      { find: /^monaco-editor$/, replacement: monacoStubPath },
      { find: /^monaco-editor\/esm\/.*\.worker(\?worker)?$/, replacement: monacoWorkerStubPath },
      { find: /^monaco-editor\/esm\/vs\/editor\/editor\.main(\.js)?$/, replacement: monacoEntryPath },
